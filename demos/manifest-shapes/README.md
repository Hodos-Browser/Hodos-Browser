...
These are the **canonical copy** — one home, not duplicated into a test folder. Both parsers read
these same files:

- `rust-wallet/src/manifest.rs :: parse_manifest` (via `include_str!`)
- `cef-native/src/core/ManifestFetcher.cpp :: ParseFromJson`

Both re-parse the same bytes at runtime (Rust fetches and embeds the manifest in the 202; C++
re-parses it to build the modal), so **a fix in one layer with a test driven by the other proves
...
| `brc73-babbage-legacy.json` | Deprecated `babbage` namespace only. | 2 protocols. Must still parse. |
...
| `hodos-legacy-permissions.json` | Our own non-standard top-level `permissions` shape (`PERMISSION_UX_DESIGN.md` §5). Zero adopters in the survey, but it is ours. | Still parses. Regression guard. |
| `unrecognised-shape.json` | Valid JSON, plain W3C web-app manifest, **no** permission namespace at all. | ⛔ **`domain_approval`, never a permission-free itemised bundle** (`P0.8-A3`) — this is the defect the phase exists to fix. |
...
figure is shown as information and never written.
...
`development-docs/TICKET_brc73_group_permissions_manifest.md` §3.
