...
**Created:** 2026-06-24 / **Status:** ✅ IMPLEMENTED / SHIPPED (Commit 1 `f2fb474`, Commit 2 `2de43cd`; live in beta.23+) / **Owner:** DevOps + wallet owner sign-off
...
> **✅ WHAT SHIPPED (2026-07-09).** Both commits landed and are live: the exit-sequencing core (Commit 1) **and** the money-DB atomicity hardening (Commit 2) — the two money-DB sub-items are **landed, not pending**. **Owner decision override:** shipped with **`PRAGMA synchronous=FULL`, NOT `NORMAL`** as the spec body (§3/§4) proposes — a deliberate beta.23 durability hardening for the money DB. Treat the `synchronous=NORMAL` recommendation below as superseded; the rest of the spec matches what shipped. This graceful-exit path now underpins the **live Windows silent updater's deterministic image-lock release** (silent auto-update proven live beta.25→beta.26 through the two-process profile picker). Anchors below (file:line) were not re-verified this pass — treat as drift-risk.
...
2. ~~**Do NOT set `PRAGMA synchronous=FULL`.**~~ **SUPERSEDED — shipped as FULL.** For a *clean* `exit(0)` after a WAL checkpoint, NORMAL is crash-safe for committed transactions and the pre-exit checkpoint forces the fsync. FULL only matters for OS-crash/power-loss, which this change does not affect. *However, the owner chose to ship `synchronous=FULL` (beta.23) — a deliberate money-DB durability hardening that trades a small per-commit fsync cost for OS-crash/power-loss safety. The as-shipped value is FULL, not NORMAL.*
...
### The second C++ kill path — ✅ CONFIRMED DEAD (2026-06-24)

There is a *separate* hard-kill-only path: `WalletService::stopDaemon()` → `cleanupDaemonProcess()` (`WalletService.cpp:769-782`) calls `TerminateProcess(daemonProcess_, 0)` directly with no `/shutdown` POST first. **Confirmed dead/legacy:** the shipped wallet process is launched by `LaunchWalletProcess` (`cef_browser_shell.cpp:3401`, CreateProcess + Job Object) and stopped by `StopWalletServer` (`:3503`) — **not** by `WalletService::startDaemon`. The many `WalletService walletService;` instances in `simple_handler.cpp`/`AddressHandler`/`IdentityHandler` are short-lived **HTTP-client** objects (getWalletStatus, sendTransaction, …); none call `startDaemon()`, so `daemonRunning_` is never true and `cleanupDaemonProcess`'s `TerminateProcess` never fires. No routing needed — `exit(0)` via the live `StopWalletServer`→`/shutdown` path is fully effective. **macOS mirrors it:** `StopServers` (`cef_browser_shell_mac.mm:5110`) POSTs `/shutdown` then `stopPid(g_wallet_server_pid, 5000, …)` polls `waitpid`, SIGTERM only on overrun — same graceful-first 5s semantics.

> **✅ IMPLEMENTATION STATUS — Commit 1 LANDED (`f2fb474`, 2026-06-24).** Exit-sequencing core implemented + smoke-validated on Windows (dev DB, isolated): full process exit **~3.4s** (< 5s budget; the Monitor join reliably hits its cap because tasks are mid-network-await at shutdown — the designed WAL-safe fallback), sequence logged in order, `-wal` 1.18 MB → 0, DB reopens clean (`/health` ok, no recovery needed). **Refinement vs spec §2:** Monitor-join cap set to **2s, not 3s** — the smoke showed 3s + the idle ~1.2s drain left too little headroom under the C++ 5s `WaitForSingleObject`; 2s restores ≥1.4s margin. **Commit 2 LANDED (`2de43cd`, 2026-06-24)** — money-DB atomicity hardening: explicit `synchronous=NORMAL` (smoke-confirmed `Synchronous: 1`) + `unchecked_transaction()`/`commit()` wraps on the four ops (`add_transaction` createAction hot path; `cleanup_failed_sending_impl`; `recover_transaction`; `task_fail_abandoned` both sweeps via per-iteration txns). Behavior-preserving (BEGIN/COMMIT around existing statements; warn-and-continue kept; early-return → rollback). Verified no nesting; build clean; clean-exit regression intact. Still TODO: the updater-side wait-for-PID/lock-release belt-and-suspenders (lands with the Windows updater); full test matrix §6 on macOS (and deeper runtime exercise of the recovery wraps + a real-createAction atomicity test).
