...
9. **Visual indicator on every auto-approved payment.** Hodos's existing **tab payment badge animation** (green-dot on the tab triggered by `payment_success_indicator` IPC; chain at `HttpRequestInterceptor.cpp:1656-1681` → `simple_render_process_handler.cpp:1020` → `useTabManager.ts:141`) fires for every successful auto-approved payment. **This must be preserved end-to-end as handlers are rewired through the permission engine**, and **must also fire for V8 shim payments** (`window.CWI` / `window.yours` / `window.panda`) so users never miss a payment going out without some signal. This is the load-bearing safeguard against a malicious site spamming auto-approved payments under the user's nose — even when no popup appears, the green dot does.
