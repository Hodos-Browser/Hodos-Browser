...
- [~] **Step 6 in progress** — per-gate cadence (each commit flips one decision type from inline to engine-driven and is independently smoke-testable against a real site):
  ...
  - Future: Commit E — scoped grants (ProtocolUse / BasketAccess / CounterpartyUse). Commit F — ProtocolUse divergence resolution (the divergence flagged in Commit A's smoke). Each lands with its own site-specific smoke verification.
