...
- [~] **Step 6 in progress** — per-gate cadence (each commit flips one decision type from inline to engine-driven and is independently smoke-testable against a real site):
  ...
  - **Post-E critical fixes (2026-05-13/15)**, each addressing a real flow-breaking bug surfaced by SocialCert testing: `1d37cc5` verify-silent (read-only crypto endpoints don't gate), `3f59de9` POST arg order (Always-allow writes were silently failing), `d742fc5` wildcard keyId + CounterpartyUse lookup (Always-allow now persists across session-rotating keyIDs), `87470ac` cache no-poison (DomainPermissionCache no longer caches fetch-failure-derived "unknown" results), `0e3dd1a` per-row try/catch (one bad legacy row no longer aborts scan), and **`83935eb` — THE bug — null-safe JSON accessors** (every V18 lookup was throwing on null counterparty fields; Always-allow appeared not to stick). Plus `b9124bb` reqwest 8s timeout on the WoC revocation check so `/acquireCertificate` doesn't hang for 45s when WoC is slow.
  - **Step 6 declared closed (2026-05-18).** Commits A through E + post-E fixes cover all the engine-rewire work. Originally-planned Commit F (ProtocolUse divergence resolution from Commit A's shadow-mode notes) is now folded into Phase 4's comprehensive demo flow Pillar 2 — Commit E migrated ProtocolUse to the engine in the same change that would have surfaced the divergence, so verification happens naturally during demo construction. See [Phase 4 README](../phase-4-demos/README.md) "Comprehensive demo flow" section.
  - Payment-scope sequencing follow-up (the dormant engine logic in Commit E — `paymentScopeKindMissing` field with tests, but not populated by C++ for Payment kind) deferred to a post-Phase-1.6 polish pass. The architectural prerequisite is the indexer resilience work in Phase 1.6, since the sequencing flow requires the C++ approve handler to re-evaluate gates rather than just re-issue, which means more rust calls that benefit from the same timeout/fallback discipline.
