...
**Scope.** Migrate the broadcast pipeline (`handlers.rs::broadcast_to_arc` / `broadcast_to_taal_arc` / `broadcast_to_gorillapool` / `broadcast_to_whatsonchain` — fn defs around lines 8670-8773 as of 2026-05-20; cited line numbers drift ~6-9 lines vs the original 1.6c kickoff so prefer function-name references) to `Services.broadcast_beef`. Adopts the `5s + 50ms/KiB cap 30s` adaptive soft timeout and `moveServiceToLast` demotion.

**Sub-commits.**
- **1.6d.D-1** — Wire `broadcast_transaction` through `services.broadcast_beef`. Add `services: &WalletServices` param to broadcast_transaction (18 call sites updated to pass `&state.services`). Preserve behaviors as wrapper logic: fatal-error short-circuit (`is_fatal_broadcast_error` post-hoc on Err body), ARC-409-timeout → `services.tx_status` confirmation, ARC merkle-proof caching under the provider-returned txid, broadcast_status='confirmed' update on MINED, and txid-collision detection (BEEF ancestry vs CVE-2026-40069 collision). Raw tx callers are auto-wrapped in a minimal V1 BEEF before being handed to the chain. **Legacy `broadcast_to_*` helpers are preserved as dead code** (`#[allow(dead_code)]` shim `_broadcast_transaction_LEGACY_for_revert_safety`) for revert safety.
- **1.6d.D-2** — Clean up. Delete the four legacy `broadcast_to_*` helper functions and the `_broadcast_transaction_LEGACY_for_revert_safety` shim. Pure cleanup, easy revert.

**What changes.** Broadcast is now a chain: GorillaPool ARC → TAAL ARC → GorillaPool MAPI → WoC. On soft timeout per provider, the provider gets demoted and the next one tries. Soft timeouts are payload-aware — a 100 KiB BEEF gets 5s + 5s = 10s soft timeout per provider instead of the current 30s hard ceiling per provider. Per-provider 3× retry-with-exp-backoff (`MAX_BROADCAST_ATTEMPTS`) is **dropped** — chain advancement replaces it. The chain itself is single-shot per provider; the soft-timeout/demotion model is the new resilience layer.

**ANNOUNCED_TO_NETWORK policy.** Hodos diverges deliberately from canonical wallet-toolbox. Canonical (`bsv-blockchain/wallet-toolbox/src/services/providers/ARC.ts`) treats ANNOUNCED as primary-broadcast success; Hodos treats it as not-success in `interpret_broadcast_response` so the `ProviderCollection` chain advances to the next broadcaster. Rationale: ANNOUNCED is step 5 of 10 in ARC's lifecycle ladder — INV sent, no peer has accepted — and today's `handlers.rs::broadcast_transaction` already falls through to fallback broadcasters on this signal (preserves existing behavior across the 1.6d.D refactor). See memory `reference_arc_tx_status_ladder` for the full ladder and decision rationale.

**What does not change.** Primary remains GorillaPool ARC. TAAL ARC stays fallback (memory `project-taal-arc-unreliable-for-primary`). The hardcoded TAAL key stays where it is (memory `project-taal-arc-key-hardcoded`). 409/460-469 error handling is preserved (now lives in `arc_gorillapool.rs::interpret_broadcast_response`). SEEN_IN_ORPHAN_MEMPOOL / DOUBLE_SPEND_ATTEMPTED / REJECTED / MINED_IN_STALE_BLOCK all remain rejection paths that surface as `IndexerError::ProviderStatus`.

**Verification.** `cargo build` clean. `cargo test` covering the new ANNOUNCED→Err branch (`broadcast_announced_to_network_is_err_so_chain_advances`) plus the pre-existing `adaptive_soft_timeout_for_payload` formula tests. Manual: broadcast small tx (1 KB), confirm 5s soft timeout per provider. Manual: broadcast large tx (50 KB), confirm 7.5s soft timeout per provider. Manual: simulate ARC GP slow → confirm TAAL takes over after the first soft-timeout. Confirm subsequent broadcasts in same session try TAAL first (demoted GP is at tail). Confirm broadcast still succeeds in all scenarios. **Smoke-pass priorities**: PeerPay self-send (canonical wallet send), BRC-121 paid retry on `now.bsvblockchain.tech` (verifies broadcast_nosend inherits the new chain).

**Risk mitigation.** Broadcast is load-bearing — every outgoing tx hits this path. Test extra carefully against the canonical sites' tx flows. Roll back means reverting two commits (1.6d.D-2 first, then 1.6d.D-1); broadcast falls back to existing pre-Services pipeline preserved verbatim in `_broadcast_transaction_LEGACY_for_revert_safety`.
