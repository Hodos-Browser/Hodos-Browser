...
- [~] **Stability soak + crash-rate gate.** **Windows 2026-08-10: 140 page loads across 14 passes, ZERO renderer crashes**, `regression_soak.py`. ⚠️ **Crashes are detected by PROBING, not by reading the log** — there is **no `OnRenderProcessTerminated` handler anywhere in this codebase**, so a crash writes nothing to `debug_output.log` and a log-grepping soak would report a confident zero forever. A dead renderer cannot answer JavaScript, so a probe that returns nothing is the crash signal. ⛔ **This is an ABSOLUTE figure, NOT the delta against M136 the row asks for**: the 136 build is not installed here and we ship no telemetry, so there is no baseline to subtract. Do not quote it as a comparison. **[ ] macOS owed. [ ] the vs-136 comparison remains unobtainable without installing 136.**
...
- [~] **Farbling × adblock (Q2 T1–T8)** — **T1/T2/T5/T6/T7/T8 GREEN on Windows 2026-08-10**, `q2_farbling_adblock_check.py`. **macOS: T1/T2/T5/T6/T7/T8 GREEN 2026-08-12** (T2 after the cosmetic stub was implemented + a second `#ifdef _WIN32` removed at the injection call site). **[x] T3 GREEN 2026-08-12 (owner-driven)** — three YouTube videos, **no pre-roll and no mid-roll observed**, with both C++ mechanisms confirmed independently from the log in the same window: scriptlet pre-cached for the watch URL (`34283 chars`) and `AdblockResponseFilter: 1 ad keys renamed` firing repeatedly, including in an **899,814-byte** player-config response. ⚠️ **Honest limit — the causal control was NOT run:** we showed the mechanisms fired and that no ad appeared, *not* that an ad does appear with adblock off, and YouTube does not serve ads on every video. "Consistent with working", not "proven causal"; a 2-minute engine-off reload would close it. **⛔ T4 is KNOWN RED**
  - **T2**: injection verified **per mechanism**, because cnn.com and youtube.com exercise different ones and each is the other's control — cnn.com (`generichide=false`, 465 selectors, 0 scriptlet bytes) gets `<style id="hodos-cosmetic-css">` at 2,190 bytes matching a real engine selector (`.ad`) and **not** a fabricated one; youtube.com (`generichide=true`, 0 selectors, ~34 KB scriptlet) correctly gets **no** cosmetic CSS. ⚠️ Two traps, both measured: (1) **read our `<style>` element by id, never "any stylesheet containing an ad selector"** — cnn.com's own 2 MB stylesheet contains `.zone__ads` while our 717-byte block does not, so the loose check returns a PASS attributable to the *site's* CSS; (2) **poll, don't read once** — `measure()` returns as soon as the host appears in the URL, which on a heavy site is `readyState:"loading"` with zero stylesheets attached, and the in-page poll must fit inside `measure()`'s 25 s evaluate window or the row dies looking like "the page would not load". Proves the right mechanism is selected and applied; does **not** prove an ad was removed — that is T3, and it needs a human.
...
- [x] Standard site basket (Thorough): Auth, Video/Media, News, E-commerce, Productivity, BSV. **Windows 2026-08-10 PASS 10/10**, `regression_soak.py`. Screenshots of every site written for human review, because a page can satisfy every automated assertion and still look wrong. ⚠️ **Two harness defects had to be fixed first, and BOTH reported working sites as broken**: (1) a read-once probe reported x.com and youtube.com at **0 characters** — the screenshot showed the X splash logo on black, since `measure()` returns as soon as the host appears in the URL, which on an SPA is the loading screen (6/10 → 9/10 once it polls); (2) google.com renders **147 chars** of innerText by design (logo + search box + footer, screenshot confirms a fully rendered signed-in page), so it carries a per-site floor of 100 rather than the global floor being lowered — dropping it globally would weaken nine working assertions to accommodate one sparse page, and a blank google.com still fails at 100. **[ ] macOS owed.**
- [ ] Adblock still works incl. YouTube `CefResponseFilter` ad-strip + cosmetic/scriptlet (Q2).
- [~] **FedCM** ("Sign in with Google" account chooser). **Coverage AUDITED 2026-08-10**: FedCM surfaces as `CEF_PERMISSION_TYPE_IDENTITY_PROVIDER` (1<<10). `SimpleHandler::OnShowPermissionPrompt` maps only camera / mic / geolocation / notifications / clipboard and then `if (types.empty()) return false`, so FedCM **falls through to Chromium's own account chooser** — which is the intended outcome; we do not want to reimplement it. **[ ] The live leg (an actual Sign-in-with-Google chooser appearing and working) still needs a real FedCM site + a human, and is owner-gated.** **[ ] macOS owed.**
