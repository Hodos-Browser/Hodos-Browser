...
> **Status:** PLAN — **GO-WITH-EDITS**, then **ARCHITECTURE PIVOTED to reuse the existing `wallet_call` IPC bridge** (red-team `wf_d67e8bd5-37e`, runs 1+2, 2026-06-24). **OQ-1 DECIDED: ships BEFORE `0.3.0-beta.16`.** Implementation in progress (owner away; thorough test required before the release pipeline).
