...
| **FEAT-B4** | Extensions — **DEFER (likely a next-quarter sprint).** Corrected target = **option 3, the "Vivaldi model": patch our self-built Chromium to surface Chrome extensions in our OWN custom header** — NOT CEF Chrome-style windows (which cost the custom header), and NOT blocked by "CEF" (we build from source; capability = patch-scale/maintenance, not permission). Reuses the **B1 farbling patch toolchain** (much bigger patch set against `chrome/browser/ui`, heavier per-bump rebase, near fork-level upkeep). **Research spike `w8wzfq63e` (2026-06-16) scoping real effort.** **0.4.0 stance: NO third-party extension hosting** — which also moots competing BSV wallet plugins (they can't install; the `window.yours` lock keeps our provider authoritative). | `deferred` | XL | spike `w8wzfq63e`; §7.5; `CWIShimScript.h` lock | Major security surface for a wallet browser + heavy compat testing. EIP-6963 only if EVM coexistence ever in scope (BSV has no equivalent). Implement post-beta, after B1 patch toolchain proves the rebase rhythm. |
