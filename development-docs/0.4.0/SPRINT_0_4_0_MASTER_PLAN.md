...
| **FEAT-B2-FILL** | Header React content doesn't fully fill its CEF window — small gap vs the webview. Likely CSS (html/body margin, container ≠100%/box-sizing, stray border) **or DPI-rounding**. Investigate WITH FEAT-DPI. | `pending` | S | `MainBrowserView.tsx` layout + header HWND `cef_browser_shell.cpp:~3528` | May be a DPI-rounding artifact at non-100% scale. **Observed 2026-06-19 (user — re-confirm): the gap is at the BOTTOM only, NOT the sides → a vertical/height issue (React container height ≠ 100% / bottom inset / box-sizing on height / DPI height-rounding), not a width problem. Narrows the fix considerably.** |
| **FEAT-DPI** | Multi-monitor DPI — overlay **mouse-offset** after moving between differently-scaled monitors (resize improved, hit-testing still off). **Deep-research item** (tried before, not to standard). | `pending` | L | overlay mouse mapping `SendMouseClickEvent`/`SendMouseMoveEvent`; `WM_DPICHANGED`; macOS NSPanel backing-scale | Suspects: Per-Monitor-V2 awareness, `device_scale_factor` to CEF, overlay screen-rect→DIP math, CEF mouse-event coordinate space. Cross-platform. Run a focused medium-weight research pass at this phase. **Observed behavior 2026-06-19 (user — re-confirm before designing): the bug repros ONLY when an OVERLAY is open. After a cross-monitor drag the HEADER buttons work correctly (hard-won) and the overlay itself sizes correctly to the new monitor (hard-won) — BUT when dragged from a LARGER to a SMALLER monitor, clicking an overlay button does nothing; the click only registers if the user aims slightly ABOVE the visible button. I.e. a VERTICAL hit-test offset — the effective click point lands below the cursor, and the offset tracks the scale delta. So the defect is in the overlay OSR screen→DIP mouse-coordinate mapping (`SendMouseClickEvent`/`SendMouseMoveEvent` + `GetScreenPoint`/`device_scale_factor`), NOT in resize/sizing. Direction (larger→smaller) and vertical-axis are the key clues.** |
...
- `PIPE-A4` **enables** `FEAT-B1-PATCH`, **re-scopes** `FEAT-B4`, **frames** `FEAT-B2`. *(B4 extensions = OUT of 0.4.0 scope — future sprint; see `Future-Features/`. Analysis below is retained for that sprint's kickoff.)*
- `PIPE-A1` **blocks** `FEAT-B1-PATCH` (no Blink patch can build without the CEF patch toolchain).
- `FEAT-B2-MEASURE` **must-precede** `FEAT-B2`; `FEAT-B2` **shares-work-with** `FEAT-B4` (toolbar ownership — *B4 = OUT of 0.4.0 scope, future sprint; carry this coupling into that sprint*) and `FEAT-B3` (bookmark bar).
...
| **Brave-fork verdict ⟷ B1/B4** | PIPE-A4, FEAT-B1-PATCH, FEAT-B4 | Same spike, two consequences: unblocks B1, re-scopes B4. Carry both explicitly into the B1 and B4 kickoffs. *(B4 extensions = OUT of 0.4.0 scope — future sprint; see `Future-Features/`.)* |
...
| **B2 header ⟷ B4 extensions** | FEAT-B2, FEAT-B4, FEAT-B2-MEASURE | CEF Chrome-runtime (only real-extension path) forces Chrome's toolbar, colliding with a custom header. **Single joint B2+B4 decision session**, fed by measure-first. *(B4 extensions = OUT of 0.4.0 scope — future sprint; see `Future-Features/`. This coupling carries into that sprint's kickoff.)* |
...
| **3 — Feature decisions + early wins** | FEAT-B2-MEASURE → **B2 React optimization (B2-SLIM + B2-WARM)** + correctness polish (B2-FILL, FEAT-DPI deep-research). *(B4 extensions = OUT of 0.4.0 scope — future sprint; see `Future-Features/`. No B4 work in this phase.)* FEAT-B3 (UI only) ships anytime. FEAT-B1-WORKER + FEAT-B1-SEED quick wins. | B3 parallel with everything; B2 is now React-only (no native port) |
...
> **NOTE (decided):** B4 extensions are **OUT of 0.4.0 scope** — moved to a separate future sprint. Full item docs now live at `Future-Features/B4-extensions.md` (+ `Future-Features/browser-extensions/`). The findings below are retained to seed that future sprint's kickoff.
>
...
> **Research-resolved (now ratify-or-override, recommendation in §7):** Q2 (parking_lot — §7.7), Q4 (CORS + iframe-confusion — §7.7), Q5 (B2 measure-first + optimize-over-rewrite — §7.4), Q6 (B4 = EIP-6963 + defer hosting — §7.5), Q7 (B1 spoof conflict resolved at C++ layer within valid set; drop WebGL vendor/renderer — §7.6), Q8 (B1 per-profile seed defensible if eTLD+1-mixed — §7.6). **Decided 2026-06-17:** Q3, Q9, Q11, Q12, Q13, Q18 (+ Q10/Q14/Q15 confirmed) — see each below. **Still genuinely open / your call:** **Q16 only** (CEF LTS branch + bump timing — taken up right after the audit-harness discussion).
...
3. **Effective prod log level** — is Option A (`warn`) actually suppressing F2/F3 `log::info!` lines in prod builds? (F1 is `std::cout`, unprotected regardless.) → ✅ **DECIDED 2026-06-17:** fix F1/F2/F3 in Phase-0 regardless; `warn` suppresses `info!` but F1's `std::cout` leaks even so — verify during Phase-0.
...
9. **Windows DSA→EdDSA** — migrate to meet A6's own EdDSA-on-both bar (needs WinSparkle 0.9.x)? → ✅ **DECIDED 2026-06-17:** yes — upgrade WinSparkle to 0.9.x, move Windows to EdDSA; smoke-test the Windows updater after the dependency bump.
10. **TAAL key** — rotate as part of 0.4.0, or accept (rotates monthly at build time)? → ✅ **DECIDED 2026-06-17:** keep the hardcoded-key model; drop in a fresh key at broadcast time until TAAL ships an env-var/alternative. No restructure. **🕹 Arcade (2026-06-17 Slack):** live v2 endpoints found + batch=NON-atomic contract + no-X-SkipValidation captured — see `BROADCAST_AND_EXPLORER_REVIEW.md` "Live Arcade endpoints + batch semantics" before kicking off the broadcast track.
11. **CI test strategy** — reusable `workflow_call` shared between ci.yml and release.yml, or duplicate? Where do C++ GoogleTest (network FetchContent) and crypto coverage (tarpaulin Win vs Linux) run hermetically? → ✅ **DECIDED 2026-06-17 ("do it right"):** shared reusable `workflow_call` (single source of truth, no duplication); pin/vendor GoogleTest (no live FetchContent download in CI); Rust coverage on Linux (tarpaulin's native platform); C++ tests run per-OS in the matrix.
12. **Frontend pyramid** — build the missing Vitest unit layer or formally retire `DevOps-CICD/TEST_PLAN.md` §3? → ✅ **DECIDED 2026-06-17 ("don't be lazy, do it right"):** build the Vitest unit layer for the frontend (hooks, wallet bridge, etc.). Do not retire §3.
13. **Appcast safety** — decouple appcast generate/push from build so a bad appcast can't auto-deploy? Soften Apple notarization hard-fail (currently blocks ALL releases on an Apple outage)? → ✅ **DECIDED 2026-06-17 ("do the right thing"):** decouple appcast *publish* from the build (build produces it; a deliberate step publishes it live); soften the Apple notarization hard-fail so an Apple outage doesn't block non-Mac artifacts and is retryable.
14. **macOS silent-update vs 2026 Sparkle CVEs** — *RECONCILED in `DevOps-CICD/AUTO_UPDATE.md` §4:* CVE-2026-47122 (appcast-item injection) + CVE-2026-47121 (delta traversal) are unpatched through 2.9.1 but **LOCAL-only** (need existing code execution; EdDSA+HTTPS block remote) → **not a hard blocker.** Mitigate with **binary-deltas OFF** + monitor for a patch. Remaining call: confirm deltas-off is acceptable and proceed with silent. → ✅ **CONFIRMED 2026-06-17:** silent auto-update, binary-deltas OFF.
15. **Branch/sprint strategy** — how to sequence/branch this unified sprint vs `feature/brc121-phase1`; confirm Phase 3 ordinals stays deferred. → ✅ **CONFIRMED 2026-06-17:** Phase 3 (1Sat ordinals) runs *after* the 0.4.0 sprint (between internal beta and public release, per §12).
16. **🆕 CEF branch strategy (from §7.3)** — ~~adopt the CEF LTS line (target M150)~~ → ✅ **DECIDED 2026-06-17:** target **CEF stable M149** (`149.0.3` / Chromium `149.0.7827.115`, verified live from `cef-builds.spotifycdn.com/index.json`). **⚠️ Correction:** CEF publishes **only stable + beta — there is NO CEF LTS/Extended-Stable channel**, so the "pin to LTS (M150)" premise was false. Conservatism = pin a stable milestone + pull its security point-releases. **CEF bump pulled EARLY** (closes M136 zero-patch gap + is B1's patch base). Distributed-build (Siso + third-party REAPI, not reclient) = **deferred beyond 0.4.0.** Full sequencing in `ORCHESTRATION_PLAN_0_4_0.md`.
...
1. **⚠️ CORRECTED 2026-06-17:** CEF has **no LTS/Extended-Stable channel** (only `stable` + `beta` on the CDN). **Target the newest CEF *stable* milestone** (currently **M149**, `149.0.3`), pin it, and pull its security point-releases. (If extended-stable conservatism is ever needed, self-build CEF from a *Chromium* extended-stable branch — extra toolchain work, not planned for 0.4.0.) Re-verify the exact build from `cef-builds.spotifycdn.com/index.json` at bump time — never a wiki.
