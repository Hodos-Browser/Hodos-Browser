...
- **B2 — DOWNGRADED to Low/Medium after reachability check (2026-06-12).** The cross-origin-iframe escalation is **largely mitigated** but not conclusively closed:
  ...
  - **The vulnerable code is the direct-fetch path only** (`extractDomain` @ **HttpRequestInterceptor.cpp:5032-5074**, called from `GetResourceHandler` at **3736 + 3869** → `AsyncWalletResourceHandler` — drift-corrected 2026-06-17, was cited 6080/4885), which attributes by **main frame**. **Note (2026-06-17 spike):** `GetResourceHandler` (sig at 3639) *already receives* `CefRefPtr<CefFrame> frame` and just ignores it — so per-frame attribution is a self-contained fix (thread `frame` into `extractDomain`, prefer `frame->GetURL()`), NOT a plumbing job. It's reachable only by a *raw* `fetch('http://127.0.0.1:31301/...')` that bypasses the shim — i.e. a malicious iframe.
...
| **B2** | 🟠 **High** *(reachability HIGH — confirmed 2026-06-14)* | `cef-native/src/core/HttpRequestInterceptor.cpp:5032-5074` (`extractDomain`), called from `GetResourceHandler` (3637; `frame` already in sig at 3639) at 3736/3869 | **Cross-origin iframe origin confusion.** `extractDomain` derives the origin from the **main frame** URL only. A cross-origin iframe (ad slot, third-party embed) issuing a direct `fetch('http://localhost:31301/createAction')` is attributed to the **top-level approved domain** → inherits its caps, identity-key disclosure, and scoped grants. Skeptic confirmed the code behavior but flagged this is the **legacy direct-fetch** path. | **Open question before rating:** is the direct-fetch path still reachable for external pages post-2.5 (page CSP `connect-src`)? Does the primary **IPC-bridge** path share the same main-frame origin determination, or is the shim per-frame? Per-frame origin attribution + a frame-origin check is the likely fix. **Investigate reachability first.** |
