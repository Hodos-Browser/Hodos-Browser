...
### b2 — Site-info hub + management UI + connection badge  · RISK: LOW · ✅ DONE (smoke-passed)
> **Progress (2026-06-21):** split into **b2a (hub shell)** + **b2b (permission management)**, both built + adversarially reviewed + smoke-passed (camera enforce/persist confirmed live).
> - **b2a** — `SiteInfoOverlayRoot` + `/site-info` route; new role `"siteinfopanel"` cloned from the **download panel** (light MA_NOACTIVATE + installed mouse hook — the hub has no text input) with the **bookmarks LEFT-anchor math**. TuneIcon **replaces** the passive lock (`MainBrowserView.tsx`), color = connection state. Connection badge (reuse `securityState`), Shields delegating toggle (`usePrivacyShield`), "This site's data" (→ new tab `tab_create`, `?domain=` rides along), "Manage Wallet Permissions" (→ existing `edit_permissions` overlay via new `open_wallet_permissions` IPC). Design-review settled two non-mechanical calls: light WndProc pattern (not bookmarks' keyboard machinery) + **no Esc** (click-outside close). Code-review caught one real bug — **toggle defeated by its own mouse hook** (hook hides on the same TuneIcon click, async IPC re-shows) — fixed with a `g_siteinfo_last_hide_tick` guard. Smoke-polish: **pre-warm at boot** (first-click header-repaint fix), **new-tab** for site-data (was active-tab), **shields stale-on-reopen** fixed via a `refreshKey` added to `usePrivacyShield` (also fixes the existing right Shield panel).
> - **b2b** — segmented **Allow | Block | Ask** per capability (camera/mic/location/notifications/clipboard) + "Reset permissions for this site", **collapsed by default** (expandable, indented rows — fits the fixed 480px panel; native `<select>` ruled out — OSR `OnPopupShow/Size` are stubs). New IPC `site_permissions_get/set/reset` + `site_permissions_response` over the existing `SitePermissionStore` (`GetAllForHost`/`SetState`/`ResetDomain`) + `useSitePermissions` hook. Review verified the make-or-break: hub set/get keys the **same store row** the b1 gate enforces (`NormalizeHost` collapses bare host + full origin identically); `Ask` deletes the row; no cache (enforcement reads `GetState` directly → immediate); **Block authoritative over an active "allow once" session grant**. **Known nuance:** after "Allow this time", the hub shows "Ask" (it displays the persistent store value, not the ephemeral session grant) — cosmetic, by design.

(Original b2 plan below.)
### b2 (original plan) — Site-info hub + management UI + connection badge  · RISK: LOW
