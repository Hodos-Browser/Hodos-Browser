...
**Note (why the old IPC is torn out):** today's farbling seed is delivered by the `fingerprint_seed` **IPC** (`simple_handler.cpp:7609` → `simple_render_process_handler.cpp:1131`, into `s_domainSeeds` — **re-verified 2026-08-05**; was `:7515`→`:1198`), which is already cross-platform CEF process messaging. It is being **deleted** in the M1 teardown (TD-3, `Q2_farbling_adblock.md` §3) not because it fails to reach Mac, but because (i) it is a per-message *push* that can arrive **after** first script (the same async-timing hole as option (b) above), and (ii) it seeds `s_domainSeeds` **per-URL**, not the persistent **per-profile** seed model B1 requires. C2 replaces it with a seed resident at/near process spawn. If C2 lands on a CEF IPC/mojo channel, delivery stays cross-platform exactly like the old IPC was — the teardown is a timing + seed-model change, not a cross-platform→platform-specific regression. ✅ **This is exactly what happened: FB-1 closed 2026-08-05 on (A′)**, CEF's existing per-frame mojo channel (already cross-platform, same as the old IPC) **plus a lazy `[Sync]` pull on first farbled API call** — which is the direct answer to this doc's first-paint timing objection, and the reason option (b)/cmdline-nonce was rejected. Mac inherits the channel for free; Mac still owns argv/ordering *verification*.
