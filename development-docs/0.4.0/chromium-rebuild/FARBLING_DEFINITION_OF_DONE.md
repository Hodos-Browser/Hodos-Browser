...
| H9 | ~~`regression_soak.py`'s exemption blind spot~~ | — | — | ✅ **DONE 2026-08-15.** Every row is now labelled `farbled`/`EXEMPT` with an `N/M had farbling ON` summary and a warning that exempt rows test stability, not farbling. Added `cnn.com` (heavy third-party iframes) and `openstreetmap.org` (canvas/WebGL). **4/10 farbled → 7/12.** ⚠️ The labelling itself had to be fixed twice: match the URL's real hostname (the basket's `host` field is a substring — `www.google.com` is exempt, bare `google.com` is not), and match subdomains one-directionally (`accounts.youtube.com` being exempt does not make `youtube.com` exempt). |
| H10 | ~~Replace the Stripe widget target~~ | — | — | ✅ **DONE 2026-08-15.** Candidates probed rather than guessed. Now **four vendors, all green**: Turnstile (token issued), Stripe payment element (11 iframes, `js.stripe.com`), Braintree drop-in (10 payment iframes across `braintreegateway`/`paypal`/`pay.google`), reCAPTCHA on a third-party host. `checkout.stripe.dev` dropped — zero iframes in both arms. |
| H11 | ~~Shared-hosting sites share one farbling key~~ | — | — | ✅ **FIXED 2026-08-15**, option B. `MultiLabelSuffixes()` extended with 33 hosting suffixes. Proven red→green on the same box: `squidfunk/microsoft.github.io` `51724237 == 51724237` → `21394036 != 9b69c283`, separation and native controls green both times, and `example.com`/`example.org` unchanged so nothing else moved. 3 unit-test cases added; 12/12 pass. |
