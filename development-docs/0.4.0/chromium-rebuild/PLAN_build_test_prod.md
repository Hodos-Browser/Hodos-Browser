...
- Run `development-docs/DevOps-CICD/scripts/build_hodos_cef.bat` (Win) / `_mac.sh` (Mac) **verbatim on the current `--branch=7103`**, `GN_DEFINES=is_official_build=true proprietary_codecs=true ffmpeg_branding=Chrome chrome_pgo_phase=0`.
...
Per outline §4: P4a Supplement + seed channel + **Canvas-first worker quick-win** → P4b Canvas+WebGL(incl. `readPixels`) → P4c Audio+Navigator(valid-set constrained) → P4d auth-domain exemption at source (`IsAuthDomain` only) → P4e OOP-worker seed plumbing. **Retire the old JS-farbling path** (M1 teardown checklist: delete injection at `simple_render_process_handler.cpp:501–547` — **✅ re-confirmed 2026-08-05 at the P4a kickoff; the inherited `:586-632` was ~85 lines stale, exactly as this note warned** — retire `FingerprintProtection.h`/`FingerprintScript.h`, remove the seed IPC chain). Value tables (§B in `Q5_full_edit_list.md`) are **no longer OPEN** — `PLAN_farbling_blink.md` exists and resolved them; the last two owner value-fills (FB-1 seed channel, FB-2 WebGL vendor/renderer) closed 2026-08-05. ⚠️ **This doc's §7.2 still says `deviceMemory ∈ {2,4,8,16,32}`; the value-owning farbling plan says `{4,8,16,32}` (FB-8) — reconcile to `{4,8,16,32}` at P4c.**
...
- **Adblock (Q2):** YouTube ad-strip via `CefResponseFilter` (`AdblockResponseFilter`) still works; cosmetic CSS + scriptlet injection still fires; the removed JS-farbling site (`:501–547`) was deleted cleanly **without disturbing adjacent scriptlet/cosmetic IPC handlers** (Q2 §ordering); blocked-count badges update. `hodos-unbreak.txt` untouched (adblock file, not farbling — I1).
