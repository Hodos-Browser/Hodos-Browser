...
> **Authoritative inputs:** the ten area docs above; outline §3/§4/§5/§6/§7; `CEF_BUILD_RUNBOOK.md` (Step 5.5/6/7); `CEF_VERSION_UPDATE_TRACKER.md`; `DEPENDENCY_VERIFICATION.md`; `WINDOWS_AUTOUPDATE_PLAN.md`; `ORG_IDENTITY_SIGNING_MIGRATION.md`; `B1-farbling-design.md`; `development-docs/DevOps-CICD/scripts/build_hodos_cef.bat` / `_mac.sh`.
...
| ID | Edit | Why | Layer | Platform | Status | Depends-on | Doc |
|---|---|---|---|---|---|---|---|
| **CEF-1** | Fork `chromiumembedded/cef` → **`Hodos-Browser/cef`**, branch `hodos/<7871>`; add `patch/patches/hodos_*.patch` + register in `patch/patch.cfg`; point build at fork via `automate-git.py --url=https://github.com/Hodos-Browser/cef.git --branch=7871 --checkout=<pin>`; **prove a no-op probe patch applies pre-compile + builds**, then remove the probe. Applied via `git apply -p0 --ignore-whitespace` (exact-context, **fail-loud, no fuzz**) | **Patch infra EXISTS and runs every build** — 114 registered `patch.cfg` entries / 115 `.patch` files on `94c1726`, applied by `gclient_hook.py`→`patcher.py` pre-compile (corrected 2026-08-05; "empty" was wrong). Net-new is only **our fork + our patches**. Still the serial linchpin blocking ALL source-level farbling | CEF | both | GREENFIELD | P1 baseline | patch_toolchain §1/§4/§8 |
| **CEF-2** | Land `DevOps-CICD/scripts/cef_patch_drift_audit.sh` (Step-5.5 hook; bash, beside its neighbours — OQ-1 (c)): per-patch `git apply --check` (read-only; **never** `patch_updater.py --reapply/--restore` — write-capable), scrape hunk-**offset** lines (soft warning), registry/orphan check, target-file-existence, **invokes** the existing `cef_dist_drift_audit.sh` (VER-5 manifest) and `cef_gn_args_gate.sh` (GN-args) rather than reimplementing them; baselines the known upstream orphan `chrome_browser_privacy_1119417`. Exit 1 = build must not start; wired as pre-build gate | Detect patch rot / manifest drift **before** a 10–12 hr build; the manifest diff feeds the auto-update apply gate | CEF / Process | both | GREENFIELD | CEF-1 | patch_toolchain §7 |
...
| **CEF-5** | **OQ-1 CLOSED as (c):** the two scripts already exist at `development-docs/DevOps-CICD/scripts/` and that path is canonical. CEF-5 reduces to **fixing the ~8 docs that cite a repo-root `development-docs/DevOps-CICD/scripts/build_hodos_cef.bat`** + add `HODOS_PATCHES.md` fork ledger | The `--url`/`--checkout` wiring must be version-controlled; the ledger is the rebase engineer's institutional memory | Process | both | GREENFIELD | CEF-1 | patch_toolchain §0/§3.2/§6.2 |
...
`hodos_farble_session_cache.patch` (C1 — ✅ landed) · `hodos_farble_seed_wiring.patch` (C2) · `hodos_farble_canvas2d.patch` (C3) · `hodos_farble_webgl.patch` (C4) · `hodos_farble_webaudio.patch` (C5) · `hodos_farble_navigator.patch` (C6) · `hodos_farble_auth_exempt.patch` (C7). **C1 creates two new Blink source files → its patch also edits `core/execution_context/build.gni` (the per-directory source list — lower churn than `core/BUILD.gn`, and the only existing file C1 touches).**
...
| ID | Edit | Why | Layer | Platform | Status | Depends-on | Doc |
|---|---|---|---|---|---|---|---|
| **C1** | ✅ **LANDED 2026-08-05** (fork `4ed200cf9`, patch `hodos_farble_session_cache`). **`HodosSessionCache : Supplement<ExecutionContext>`** — new `hodos_session_cache.{h,cc}` under `core/execution_context/` + a two-line entry in `core/execution_context/build.gni`. **NO `execution_context.{h,cc}` hook needed** (already `Supplementable`) and **not** `core/BUILD.gn` (Blink uses per-directory `build.gni`) ⇒ **C1 modifies no existing source file**. Holds `domain_key_` + `enabled_`; exposes `From(ctx)`, `MakePrng()`, `PerturbPixels()`, `FarbleAudioSample()`, `FarblingEnabledForThisContext()`. Farbles at **API-call time**, not context-creation. Attaches to `LocalDOMWindow` + **in-process** worker/worklet contexts | Native = undetectable (`.toString()`→`[native code]`), covers in-process workers (today's #1 leak: `OnContextCreated` never fires for workers) | Blink | both | NEW | CEF-1 | farbling §3/§6 |
| **C2** | **Persistent per-profile seed, OFF the renderer command line.** ✅ **Channel CLOSED 2026-08-05 = (A′)**: push over CEF's **existing** per-frame mojo channel (`cef/libcef/common/mojom/cef.mojom` + `CefBrowserFrame::RegisterBrowserInterfaceBindersForFrame`, both already in our fork ⇒ **zero Chromium patches browser-side**) at navigation commit, plus a lazy `[Sync]` pull on first farbled API call to close the first-script race. Only the final renderer→Blink setter is a Chromium patch. `profile_seed` (32B CSPRNG via **`BCryptGenRandom`** / `SecRandomCopyBytes`, NOT deprecated `CryptGenRandom`) stored in **C++ profile data** — a new `profileSeed` field in the existing `%APPDATA%/HodosBrowser/<profile>/fingerprint_settings.json` (NOT the wallet). **Browser process computes `domain_key = HMAC-SHA256(profile_seed, first-party eTLD+1)` and delivers ONLY `{domain_key, farble_enabled}` to the renderer** — the master seed **never leaves the browser** (supersedes B1-design's "renderer derives the HMAC"). **Channel = OPEN (FB-1):** default **(A) mojo / commit-params per-navigation** (farbling FB-1); alt **(B) ephemeral per-launch nonce on the child cmdline** (a throwaway, not the seed — Q1 leans this on first-paint timing-safety). First-party/top-frame keying (I4) | Login-stability across restarts (persistent) without leaking a stable machine-local secret to any local process (C2 threat model); first-party keying blocks cross-site tracking | Blink + Shell (`ProfileManager`/`SettingsManager`) | both | NEW (channel OPEN) | C1 | farbling §4; Q1 §2.2 |
| **C3** | **Canvas 2D** readback farbling. ⚠️ **Funnel corrected 2026-08-05 against the real 7871 tree:** `platform/graphics/static_bitmap_image.cc` is **NOT** the funnel (118 lines, neither path routes through it) — **do not patch it**. Hook (i) `base_rendering_context_2d.cc :: getImageDataInternal` (`:353`, the single funnel for both overloads *and* the OffscreenCanvas-in-worker path — this is what buys the P4a worker win) and (ii) the two encode call sites of `html_canvas_element.cc :: Snapshot` (`ToDataURLInternal :1312`, `toBlob :1423`; not `Snapshot :1270` itself — it has a third, non-fingerprinting caller). `measureText` = gate only. Keep the small-canvas (<65536px) LSB gate. **Does NOT cover WebGL `readPixels`** | JS canvas farbling detectable + worker-blind → login/anti-bot friction | Blink | both | NEW | C1 | farbling §6 C3 |
...
| ID | Edit | Why | Layer | Platform | Status | Depends-on | Doc |
|---|---|---|---|---|---|---|---|
| **TD-1** | Delete the JS FP injection block **`simple_render_process_handler.cpp:501–547`** (**re-verified 2026-08-05**; both the 2026-07-10 `:581–627` and the outline's `:586-632` are stale — the block moved up ~80 lines): auth-domain skip (`:505`), `s_domainSeeds` lookup, `FINGERPRINT_PROTECTION_SCRIPT` patch+inject (`:537–544`). **Keep** the adjacent scriptlet block `:487–499` byte-identical; **keep** the `window.chrome` stub `:549–573` | JS injection detectable ≥6 ways + worker-blind; native path (C3–C6) supersedes it | Shell | both | NEW (deletion) | native path live (C3+) | Q2 TP-1; farbling §6 |
...
| **TD-5** | **Re-home the per-site fingerprint on/off toggle** (`IsSiteEnabled`/`SetSiteEnabled` + `fingerprint_get/set_site_enabled` IPC `simple_handler.cpp:6285/6304`, `FingerprintProtection.h :: IsSiteEnabled` / `:: SetSiteEnabled`, now `:129`/`:141` — by symbol, since the 2026-08-05 docstring fix shifted the header +6 below `:72`) into **C7's `ShouldFarble` input B (or C7b)** — a **shipped Privacy-Shield user control**, NOT the auth allowlist. **Do NOT delete until `ShouldFarble` consumes it** (Q2 T8 gate) | Deleting drops a shipped feature with no replacement | Shell + Blink | both | NEW (destination NOW EXISTS — was blocked on the unwritten plan) | C7 / C7b | Q3 §3 (owns the re-home) |

### A.5 — Bot-signal shims re-home (NEW) — `PLAN_farbling_blink.md` FB-4

| ID | Edit | Why | Layer | Platform | Status | Depends-on | Doc |
|---|---|---|---|---|---|---|---|
| **BOT-1** | **Preserve `navigator.webdriver=false` (`FingerprintScript.h:128–133`) AND the 5-entry `navigator.plugins` spoof (`FingerprintScript.h:99–126`)** — **both** live **inside** the deleted `FINGERPRINT_PROTECTION_SCRIPT` — in a carrier independent of farbling per-site enable; **keep** the `window.chrome` stub **`:549–573`** byte-identical | These are **bot signals, not farbling** — absence/`true` `webdriver` = bot tell. Must survive **both** the TD-1 JS-block deletion **and** a per-site farbling opt-out. **Lands FIRST, as its own commit, BEFORE any teardown.** ⚠️ Measure the native values first (2026-08-05: no `--enable-automation` is passed, and auth-domain sites already run unspoofed and work) — re-home what genuinely differs, guarded-shim the rest | Shell | both | NEW | **precedes** TD-1 | farbling §6 C6 / FB-4 |
...
| `navigator.plugins` | C6 | ✅ **DONE (BOT-1, 2026-08-05)** — JS spoof **deleted**, native list kept. **No C6 work needed.** | Native = the spec'd whatwg/html#6738 list (`PDF Viewer`, `Chrome PDF Viewer`, `Chromium PDF Viewer`, `Microsoft Edge PDF Viewer`, `WebKit built-in PDF`), present because we build `enable_pdf=true`. Our spoof said `"Chrome PDF Plugin"` (pre-2021 name) — measurably wrong | **LANDED** |
| `navigator.webdriver` | BOT-1 | ✅ **DONE (2026-08-05)** — JS override **deleted, not re-homed**; native is already `false` | Re-homing would add an own-property accessor where Chrome has a prototype accessor = a tamper tell. Guarded instead by the TRIPWIRE at `cef_browser_shell.cpp`'s `remote_debugging_port` block + `farbling_probe.py` assertions | **LANDED** |
...
| WebGL `UNMASKED_VENDOR/RENDERER` | C4 | ✅ **DROP — owner-signed-off 2026-08-05.** No GPU-string map; C4's `getParameter` hook is unnecessary and **FB-6 never opens** (Mac ANGLE strings not required, must not block the gate) | random strings are *more* unique than truth. If mapped: small set of **real** GPU strings incl. **Apple-Silicon + Intel-Mac ANGLE** (I8/Q1), runtime-selected off the real reported renderer, never noise | **RESOLVED (FB-2)** |
