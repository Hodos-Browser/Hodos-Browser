...
**Cross-refs:** `CHROMIUM_CEF_BUILD_DESIGN_OUTLINE.md` §3c (FEAT-B1 / C1–C7 patch set; C2 seed channel; M1 teardown; I1/I2/I4), §4 (P4d/P6 acceptance), §6 Q2/Q3; `Q2_farbling_adblock.md` (TP-2 flags `IsSiteEnabled` re-homing as an **unresolved gap this plan must close**); `B1-farbling-design.md`; `DevOps-CICD/research/BRAVE_FORK_FEASIBILITY.md` §B1. `PLAN_farbling_blink.md` (the standalone C1–C7 patch plan) **now exists** and is source-of-truth for C1–C6 and the seed model (superseding outline §3c where they disagree); **this doc owns C7**. Code anchors **re-verified 2026-08-05** at the P4a kickoff — the 2026-07-10 numbers had drifted 48–95 lines and are retained inline as "was …".
...
Today the exemption is a **three-input decision** computed in the **browser process** on every main-frame navigation (`simple_handler.cpp :: OnBeforeBrowse` **`:7578-7615`** (was `:7484-7521`)):

```
farble THIS navigation  ⇔  GlobalEnabled  AND  !IsAuthDomain(url)  AND  IsSiteEnabled(host)
```

…and then **re-checked (and, for subframes, solely enforced)** in the renderer at injection time (`simple_render_process_handler.cpp:505`, was `:585`). The browser-side decision above fires **only for `frame->IsMain()`** (`simple_handler.cpp:7579`, was `:7485`); the renderer's `IsAuthDomain(url)` at `:505` runs **per-frame** and is therefore the *only* auth check that covers subframes — not purely redundant. Only **`IsAuthDomain`** is the "auth/OAuth exemption." `IsSiteEnabled` is the **user's per-site Privacy-Shield toggle** (a shipped feature, backed by `fingerprint_settings.json`). `hodos-unbreak.txt` is **not** farbling at all — it is the adblock **scriptlet** exemption and is **untouched by this sprint (I1)**.
...
| Layer | Mechanism | Owns | Code anchor (**re-verified 2026-08-05**) | This sprint |
|---|---|---|---|---|
| **A. Auth/OAuth farbling exemption** | `FingerprintProtection::IsAuthDomain(url)` — hardcoded C++ allowlist, ~37 entries, **exact host match** on `ExtractDomain(url)` | Farbling OFF for auth/CAPTCHA/bank/e-comm hosts | `FingerprintProtection.h :: IsAuthDomain` (`:195-276`) | **C7 re-implements THIS** |
| **B. User per-site toggle** | `IsSiteEnabled(domain)` / `SetSiteEnabled` — Privacy-Shield on/off per site, persisted to `fingerprint_settings.json` (`siteOverrides_`) | User-chosen farbling OFF for a site | `FingerprintProtection.h :: IsSiteEnabled` / `:: SetSiteEnabled` / `:: LoadSiteSettings` / `:: SaveSiteSettings` (`:129`/`:141`/`:155`/`:177`); IPC `fingerprint_get/set_site_enabled` `simple_handler.cpp:6285/6304` (was `:6191/6210`) | **C7 must RE-HOME (Q2 TP-2 gap)** |
...
**Where the decision is made today (browser process):** `simple_handler.cpp :: OnBeforeBrowse` `:7578-7615` (was `:7484-7521`) — on `frame->IsMain()` navigation, if global FP enabled and not localhost:
...
### 2.3 Delivery to the renderer — ✅ FORK RESOLVED 2026-08-05 (was a HARD FORK on C2's channel choice)

> ✅ **C2 chose (A′): push over CEF's existing per-frame mojo channel at navigation commit, plus a lazy `[Sync]` pull on first use** (`PLAN_farbling_blink.md` §4 / FB-1, owner-approved 2026-08-05). That is **per-navigation AND top-frame-keyed** — i.e. exactly branch **(b)** below, satisfying *both* legs of R2. **So C7's "one extra boolean, no new IPC" property HOLDS**, and the startup-cmdline branch (a) is dead. The fork text below is retained as the decision record; read it as history, not as a live risk.
> ⚠️ **The "no new IPC, rides on C2's payload" claim is conditional, not settled.** It holds ONLY if C2 picks a **per-navigation** delivery channel. Outline C2 (line 153) leaves the channel open between **(a)** a startup ephemeral-nonce **cmdline switch** (the `B1-farbling-design.md` §45–53 default: profile seed delivered *once at renderer startup*, `domain_seed = HMAC(profile_seed, eTLD+1)` computed *inside the renderer* per page — **there is no per-navigation payload in this model**) and **(b)** a **per-navigation** mojo / pref-on-navigation channel. Today's code is per-navigation (`fingerprint_seed` IPC in `OnBeforeBrowse:7609`, was `:7515`); this doc's simplicity claim implicitly assumes the migration keeps that model — but per-navigation is exactly what C2's threat-model fix may replace.
...
- Repo (**re-verified 2026-08-05** at the P4a kickoff): `cef-native/include/core/FingerprintProtection.h` — cited **by symbol**, since this commit's docstring fix shifted the file +6 below `:72`: `:: IsAuthDomain` (`:195-276`), `:: IsSiteEnabled`/`:: SetSiteEnabled` (`:129`/`:141`), `:: LoadSiteSettings`/`:: SaveSiteSettings` (`:155`/`:177`). The 2026-07-10 values were correct until that fix, `cef-native/src/handlers/simple_handler.cpp` (`OnBeforeBrowse` seed/disable decision **:7578-7615**, toggle IPC **:6285/6304**), `cef-native/src/handlers/simple_render_process_handler.cpp` (injection + auth recheck **:501-547**; `window.chrome` stub **:549-573**), `development-docs/0.4.0/CHROMIUM_CEF_BUILD_DESIGN_OUTLINE.md` §3c (C1–C7, I1/I2/I4), §6 Q3; `development-docs/0.4.0/chromium-rebuild/Q2_farbling_adblock.md` (TP-2, T8); `development-docs/0.4.0/B1-farbling-design.md`.
