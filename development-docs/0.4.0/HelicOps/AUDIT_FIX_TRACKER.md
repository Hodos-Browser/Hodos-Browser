...
## Wave 1 Track A closure — F5 fixed → TRACK A COMPLETE (2026-06-18)

F5 landed via the profile-manager review's R1 (`PROFILE_MANAGER_REVIEW.md`). **Fix:** `ProfileManager.h` adds inline `IsValidProfileId` (9 GoogleTests), `LaunchWithProfile` gains a cross-platform validation guard, the `profiles_switch` IPC validates at the boundary (defense-in-depth), and the macOS branch replaces `system()` with `posix_spawn` (argv, no shell) + `waitpid`.

**Deliberate divergence from the audit text** (which said *"validate `profileId` against `GetProfiles()`"*): validation is **format-based** (`IsValidProfileId` — allows `[A-Za-z0-9_ -]`, covering `Default`/`Profile_N`/legacy `Profile N`) rather than existence-based. Reasons: (a) it's a pure, stateless, unit-testable security primitive; (b) the id is also a directory name, so the charset gate doubles as path-traversal protection; (c) the existence/coherent-fallback concern is a separate, broader item (review **R7**). `posix_spawn` removes the shell entirely, so even a weakened validation can't inject. **Build:** `hodos_tests` 9/9 new (54→63 total path… see test files); Windows `HodosBrowserShell` recompiles clean. **⚠️ macOS `posix_spawn` branch is `#elif`-gated — compile-verify required on the Mac build** (captured in `MACOS_PORT_0_4_0.md`).

**Track A (F5/F6/F7/F9) is now COMPLETE.** Remaining profile work (R2/R3 restart-ordering, R5, R8, R9, UX R10–R14) is tracked in `PROFILE_MANAGER_REVIEW.md`, not here.
