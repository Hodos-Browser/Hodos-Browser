...
>
> **Created 2026-06-17. Status: living Windows→mac parity delta log.**
>
> **What shipped (2026-07-09):** Windows 0.4.0 feature work shipped publicly through **v0.3.0-beta.26** (LATEST/live). The Windows silent auto-update saga is COMPLETE + proven live on real hardware (beta.25→26 through the two-process profile picker; macOS silent proven beta.21→22). Mac Claude has since landed several parity items from this log — dropdown-button consistency (commit `3958cff`) plus earlier picker / startup-resolver / async-server sessions. This log stays the canonical Win→mac parity backlog; remaining unchecked items below are still open for the Mac sprint. Next sprint = Chromium/CEF rebuild (`development-docs/0.4.0/CHROMIUM_CEF_SPRINT_KICKOFF.md`).
...
- **🗄️ Picker MODE — SHELVED (2026-07-08/09).** The **profile picker + per-profile-wallet architecture is shelved** (owner decision: revisit way down the road with real market feedback; **the wallet stays SHARED across profiles**). Do NOT port the mac picker path as part of the current sprint. The same-process picker refactor is likewise deferred. Historical port notes kept for archaeology: the pre-window picker (neutral `.picker-cache` `CefInitialize` branch, full-window `/profile-picker?mode=window`, spawn-then-`WM_CLOSE`) lives entirely in Windows code (`cef_browser_shell.cpp` `g_picker_mode` branches + `simple_app.cpp` OnContextInitialized + `simple_handler.cpp` `profiles_switch`); macOS never got an equivalent, and none is needed while shelved. If ever revived: add a mac picker path (NSWindow hosting the `/profile-picker?mode=window` browser, no profile lock/DBs, `LaunchWithProfile`-then-quit), respecting invariant #8; macOS single-instance is the `NSApplication` reopen delegate, not the `.picker` pipe. (Anchors above unverified this pass — code not re-checked; treat as drift-risk.)
...
### Startup first-paint fix + dead BRC-100 binding removal + Windows app.manifest (2026-06-22, branch `0.4.0`, commits `0c5ce87` / `87973a3` / `957962f`)

Investigation + design doc: `development-docs/0.4.0/STARTUP_OPTIMIZATION.md`. The ~2s Windows first-paint lag was root-caused to `App.tsx:135 await brc100.isAvailable()` — a synchronous native V8 binding (`BRC100Handler`→`BRC100Bridge`, blocking WinHTTP) run on every page's mount, whose result was only logged. Removed it (+ the entire unused legacy BRC-100 binding layer) and added a Windows app.manifest. Three commits; Windows build + wallet send/receive verified.

- **(commit `0c5ce87`) Startup probe removal — CROSS-PLATFORM, BENEFITS MAC MORE.** `frontend/src/App.tsx` deletes `initializeBRC100()`/`brc100.isAvailable()`. Pure React → ports to mac for free. **⚠️ Likely a bigger mac win than Windows:** the mac `BRC100Bridge` path used **synchronous libcurl with a 10s timeout** (`BRC100Bridge.cpp` `#elif __APPLE__`, `curl_easy_perform`) on the render thread, so a cold-wallet mac launch could stall first paint even longer than Windows' ~2s. **Mac live-smoke (next mac run):** confirm the header/first paint appears promptly on a cold-wallet launch (no multi-second blank). NOTE: mac startup also blocks on `StartWalletServer`/`StartAdblockServer` (up to ~10s each, `cef_browser_shell_mac.mm:~4770`) — a SEPARATE mac-only first-paint issue (see STARTUP_OPTIMIZATION.md "macOS track": instrument-first + non-blocking backend launch + `setWantsLayer`/show-on-ready), still TODO and not addressed here.
- **(commit `87973a3`) Legacy BRC-100 binding deletion — ⚠️ MAC COMPILE-VERIFY REQUIRED.** Deleted `BRC100Handler.{cpp,h}` + `BRC100Bridge.{cpp,h}` (the latter contained the mac **libcurl** `#elif __APPLE__` branch). Removed both sources from **all three** CMakeLists lists incl. the **macOS main sources** (`~:210-211`) and **`HODOS_HELPER_SRCS`** (`~:228-229`), so the mac build no longer references them. Removed the `#include "BRC100Handler.h"` + `BRC100Handler::RegisterBRC100API(context)` call from the **cross-platform** `simple_render_process_handler.cpp` (used by the mac render helper `mac/process_helper_mac.mm`) — so mac renderers also stop registering the dead `window.hodosBrowser.brc100.*`. **On the first mac build, confirm:** (a) it compiles/links with the 4 files gone (no dangling include/CMake ref); (b) the `window.CWI`/`yours`/`panda` shim still injects on external pages (the shim block sits right after the removed registration in the same shared file — verified intact on Windows); (c) wallet UI still works (it calls the Rust API directly, never these bindings). Frontend `tsc` is clean (deleting `bridge/brc100.ts` removed a conflicting `Window.hodosBrowser` declaration; TS now resolves to canonical `types/hodosBrowser.d.ts`).
- **(commit `957962f`) Windows app.manifest — WINDOWS-ONLY, NO MAC EQUIVALENT.** `cef-native/hodos.manifest` (Win10/11 `supportedOS` GUID) merged via linker `/MANIFESTINPUT` in `CMakeLists.txt` (Windows `elseif(WIN32)` block only). macOS has no PE manifest / `supportedOS` concept (it uses `Info.plist`), and the OS-misdetection→GPU-disable failure mode is Windows-specific. **No mac action.** The mac first-paint/GPU story is the separate "macOS track" in STARTUP_OPTIMIZATION.md (`setWantsLayer:YES` on header NSView ancestors before `CreateBrowser`, show-on-ready, `--use-angle=metal` verify) — untouched here.
- **Risk / notes:** all changes are deletions or Windows-only additions; no new mac code to author. The single hard mac gate is the **compile-verify** of the binding deletion (libcurl branch + CMake refs). The startup probe removal should be re-smoked on mac for the perceived win.
