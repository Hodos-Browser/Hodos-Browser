...
| `P2-A7` | `Logger::Log` from N threads produces N intact lines, none torn | 🔴 **NO NEGATIVE CONTROL EXISTS.** Removing the lock produced **0** torn lines at 8×400, at 8 KB payloads, and at **96,000 lines** (M2.1). The cause of the production tearing is **not established** | `hodos_tests`, 96,000 lines, 8 threads | T1 | ⛔ **NOT MET** — lock kept as a UB fix only; must not be cited as fixing the tearing |
