...
| `P0.9-M5` | ✅ **A3 items 1 and 2 and A4 all PASS on macOS** (supersedes the retracted `P0.9-M1`) | Web security ON, verified on the network-service child argv. State clean (`verify` exit 0). **A3.1:** `🔔 Creating notification overlay (type: permission_request, domain: example.com) (macOS)`; overlay DOM shows the gold browser icon, `💻`, and exactly **two** buttons `["Don't allow","Allow"]`. **A3.2:** owner denied; nothing persisted; the identical fetch produced a **second** prompt event 13:07:40 with buttons restored — deny is temporary. **A4:** after answering, exactly **one** on-screen Hodos window per `CGWindowListCopyWindowInfo`; the overlay document persists empty by keep-alive design, so no click-eating layer survives. RED half: the retracted `P0.9-M1` run is itself the negative control — with `--disable-web-security` the prompt never fires at all. |
