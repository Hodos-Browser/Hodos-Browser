...
|---|---|---|---|---|---|
| `P3-A1` | The AUMID **decision** is a pure function with the right answer per state: prod+1 profile → `HodosBrowser`; prod+Profile_1 → `HodosBrowser.Profile_1`; dev → `HodosBrowser.Dev*`; picker → per Q5 | Unit test in `hodos_tests` (CEF-free, alongside `profile_id_test.cpp`). Revert the gate to `size() > 1` in the same binary → the prod+1-profile case returns "no AUMID" and the test goes RED | `hodos_tests`, release config. ⚠️ Tests the **decision**, not the taskbar — never cite this row as evidence the taskbar is fixed | T1 | ⬜ |
...
| `P3-A5` | ⭐ **The coexistence control for `P3-A4`.** With 2 profiles, the second profile's window gets its **own** taskbar button, **correctly named** | Force both profiles to the same AUMID → the buttons collapse into one, and the row goes RED. Un-register the AUMID display name → the name reverts to the exe filename | 👤 **RED ALREADY OBSERVED (M9.1a):** the **Hodos** (`Profile_1`) button reads "hodosbrowser.exe" today. 🚨 So shortcuts alone **cannot** satisfy this row — a per-profile AUMID matches no shortcut. Needs AUMID **display-name registration** (M9.1a option A) | T3 (human) | ⬜ |
...
| `P3-G11` | T0 gate, **ratcheted**: the count of process-globals used for window-scoped work (`g_hwnd`, `g_header_hwnd`, `GetAllTabs()`, `GetActiveTab()`, the global `Get*Browser()` shims) does not **exceed** today's baseline. ⭐ Lands here, drives down in beta.4 | `preflight.ps1 -NegativeControl -Only G11` injects one → gate fires | `cef-native/src` + `cef_browser_shell.cpp`, Windows tree. ⚠️ Baseline **measured by the tool**, per `HARNESS.md` §9 — never by the hand-counts in M9.4 (the G2/G5 lesson: hand counts were wrong twice) | T0 | ⬜ |
...
| **Q2** | 🚨 **The fix is now three parts, not two, and they are all-or-nothing.** M9.1a raised it: shortcuts cannot name a **per-profile** AUMID, because none exists to match. Fix #1 alone also **orphans the pin of every single-profile user**, exactly as 2026-07-06 did to yours (M7). | (i) ship **#1 gate + #2 shortcuts + #3 AUMID display-name registration (`HKCU`) + a pin decision** together; (ii) shortcuts only — ⛔ leaves Hodos/`Profile_1` still reading "hodosbrowser.exe", i.e. **does not fix what you reported**; (iii) defer #3 entirely | **(i).** It is the only option where process, shortcut, pin **and per-profile** identity all agree. ⚠️ It introduces a **registry write** (`HKCU\Software\Classes\AppUserModelId\…`) — new surface for us, and it must be cleaned up on uninstall and on profile delete |
