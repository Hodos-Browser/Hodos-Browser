...
| `P3-A1b` | 🆕 The **registration mechanism** works end to end: launching writes `HKCU\…\AppUserModelId\<aumid>` with `ApplicationName = "Hodos Browser"` | 🔴 **Before/after on the same machine:** the key was **absent** before launch (verified), present after. Same-binary control = delete the key and relaunch → it is rewritten; skip `RegisterAumidDisplayName` → it stays absent | 🎯 **DEV build** (`build/bin/Release`, `HODOS_DEV=1`), so the identity is `HodosBrowser.Dev`. ⛔ This proves the **mechanism**, NOT the production string or the shortcut match — those need a real install (`P3-A4`/`A6`) | T2 | 🟢 **GREEN** — `HodosBrowser.Dev` → `Hodos Browser`, icon path set; log: `AUMID set: HodosBrowser.Dev (profile Default)` + `[AUMID] Registered display name` |
