...
| **Q2** | ✅ **ANSWERED — ship as one change.** ⚠️ Part #3 CHANGED after the owner's test: the registry mechanism was refuted (M10) and replaced by a **per-profile Start Menu shortcut**, which is what Chrome does | Shipped: **#1** always set the AUMID · **#2** installer `[Icons]` declare `HodosBrowser` · **#3** `EnsureProfileShortcut` per non-Default profile · **#4** pins → release note (Q4) | ⚠️ **Live consequence to accept or reject:** each non-Default profile now adds a **Start Menu entry** (`Hodos Browser - <name>`). The alternative is one shared identity — one correctly-named button for all profiles, but **no** per-profile buttons. Owner has the call |
