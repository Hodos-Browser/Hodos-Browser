...
**Workstream:** WS1b(a) · **Ticket:** `../TICKET_stray_log_in_install_root.md` · **Status:** 🟡 CODE COMPLETE — T2/T3 rows owed
...
| `P0-A1` | Full session → **no** new file in `{app}` | 🔴 **observed**: shipped beta.2, live, `{app}\debug_output.log` grew **1,896,960 → 2,118,968 B** between 14:15 and 15:26 on 2026-08-18 while the owner browsed | `{app}` = `%LOCALAPPDATA%\HodosBrowser\`, **not** the build dir | T3 | ⬜ **GREEN OWED** — needs a session on the new binary |
| `P0-A2` | The same events **do** appear under `%APPDATA%\…\logs\` | Suppress the `Logger` call → they vanish there too | Both halves. **A silent no-op looks identical to a fix** | T3 | ⬜ **GREEN OWED** — pairs with `P0-A1`, same run |
...
| `P0-A4` | `{app}\debug.log` absent across ≥10 launches | 🔴 **observed**: present on the shipped build — 351 B, contents are only 3× `TabManager initialized` | ⚠️ Ordering-dependent. **One clean launch is not evidence** | T3 | ⬜ **GREEN OWED** — needs ≥10 launches of the new binary |
...
- [ ] Every evidence row GREEN **and** its RED observed — **5 of 9 complete**; `P0-A1`/`A2`/`A4` (T3)
      and `P0-A5` (T2 upgrade install) owed, `P0-S1` closed as NOT REPRODUCED
