...
| `P0.5-B1` 🔴**NEW (Mac E3 2026-08-22)** — self-nav role-guard asymmetry on the BRC-100 overlay IPC family | The self-nav role gate (`role_∈{notification,brc100auth}`) that `ee8f836` added to `add_domain_permission`/`_advanced` (`simple_handler.cpp:4994/5086`) is **also** applied to the sibling privileged arms `grant_scoped_permission` (`:5197`), `approve_cert_fields` (`:5303`), `approve_identity_key_reveal` (`:5396`), `approve_key_linkage_reveal` (`:5444`) — so a self-navigated `tab_<id>` at `127.0.0.1:5137/brc100-auth?…` cannot write a persistent grant for an attacker-chosen domain | ⛔ **Pre-fix (current tree): those arms have NO role check** — a self-navved tab renders the prompt from `window.location.search` (`BRC100AuthOverlayRoot.tsx:381-475,532`) and one Allow click → header-free first-party POST `/domain/permissions/*` → Rust `domain_trust_mw` (`main.rs:44-66`) writes the grant (no `X-Requesting-Domain` backstop). CODE_READING; the general substrate was MEASURED by panel #3 on `add_domain_permission`. Negative control: same tab firing `add_domain_permission` is REFUSED (`:4995`) while the sibling is written = asymmetry | The **absence** of the Layer-2 role check on ~5 shared arms all reachable from role `tab_<id>` via the shared frontend; cross-platform (shared `simple_handler.cpp` + shared `BRC100AuthOverlayRoot.tsx`), surfaced by the mac E3 role lens | T1/T2/T3 | ✅ **FIXED, GREEN + RED observed. §4y.** Owner-approved 2026-08-22. One Layer-2 choke in `OnProcessMessageReceived` (`hodos::IsGrantApproveMessage(name) && !IsApprovalOverlayRole(role_)`, predicates in new header-only `include/core/IpcAuth.h`); the two per-arm dupes removed; `domain_permission_invalidate` given a separate `tab_*`-only denial (legit from settings/wallet). Unit test `tests/ipc_role_guard_test.cpp` (9) GREEN; **RED observed** (weaken `IsApprovalOverlayRole`→`true` → `SelfNavTab.*` + `ApprovalOverlay.RejectsEverythingElse` fail; restored→green). Shell compiles; suite 223-pass/1-skip. ⚠️ live T3 approval smoke on a dev build still owed (brc100_auth_response is money-path; my no-regression evidence is CODE_READING + unit). Commit `[P0.5-B1]` |
...
### 4y. `P0.5-B1` — self-nav role-guard asymmetry on the BRC-100 overlay IPC family (Mac E3 2026-08-22, FIXED 2026-08-22)
...
**FIX — landed 2026-08-22, owner-approved (was reported-not-fixed per HARNESS §6; owner then approved the
production change).** One Layer-2 choke at the top of `SimpleHandler::OnProcessMessageReceived`, right after the
Layer-1 internal-origin gate:
`if (hodos::IsGrantApproveMessage(message_name) && !hodos::IsApprovalOverlayRole(role_)) return true;`
The three predicates are pure + header-only in **`cef-native/include/core/IpcAuth.h`** (unit-testable without CEF,
same class as `PortConfig.h`): `IsGrantApproveMessage` (the 7 arms), `IsApprovalOverlayRole` (`{notification,
brc100auth}` — the exact allowlist the old per-arm guard used), `IsTabRole` (`tab_<id>`). The two now-redundant
per-arm role checks on `add_domain_permission`/`_advanced` were **removed** (single spelling — a second copy of the
authz rule is how the siblings were missed). `domain_permission_invalidate` is **not** in the family allowlist
(it is legitimately sent from the settings AND wallet panels — verified against `ApprovedSitesTab.tsx` /
`DomainPermissionsTab.tsx`); it gets a separate `IsTabRole`-only denial so only a self-navved tab is refused (low:
DoS/re-prompt).

**GREEN + RED.** Unit test `tests/ipc_role_guard_test.cpp` (9 cases) in `hodos_tests`. **RED observed** by weakening
`IsApprovalOverlayRole` to `return true` (the pre-fix "no gate on the siblings" state), rebuilding, running:
`SelfNavTab.IsDeniedEveryPrivilegedArm`, `SelfNavTab.OtherNonApprovalRolesAlsoDenied`, and
`ApprovalOverlay.RejectsEverythingElse` fail; restored → all green. Production compile confirmed (the full
`HodosBrowserShell` app built clean with the choke in `simple_handler.cpp`); suite 223-pass/1-skip. Commit
`[P0.5-B1]`.

⚠️ **Still owed — a live T3 approval smoke on a DEV build.** Gating `brc100_auth_response` to the approval-overlay
