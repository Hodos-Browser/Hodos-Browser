...
| `P3.5-Z5` | 🚨 **The DISMISS half, and the row this table never had.** Closing an overlay that was opened in window B leaves B visible and in front — the same guarantee as `Z1`, one interaction later | 👤 **OWNER-OBSERVED on `4dbdf61`**: clicking outside the wallet to close it makes B vanish. ⛔ The RED is real but its **discriminator is UNMEASURED** — the owner's words are *"minimizes **or** goes behind"*, and those have different causes | ⛔ **`Rect`/`IsIconic` vs `Z`.** `-32000` or `IsIconic` = MINIMIZED; B's `Z` below A's with the rect unchanged = BEHIND. ⚠️ Needs the **owner's profile** — a CDP rig cannot dismiss the wallet, because `g_wallet_overlay_prevent_close` never clears without a usable wallet (K22) | T3 | 🟢 **GREEN, owner-confirmed (K27).** RED was **BEHIND, not minimized, and it took the keyboard** (K23). ⭐ 👤 The owner then identified level 1 as a symptom fix — 📏 the drop still happened and was undone 233 ms later (K25) ⇒ **root fix taken (K26)**, ownership follows the requesting window, both patch helpers deleted. 📏 Post-fix the primary never reaches the front in any sample |
...
> ⛔ **SUPERSEDED IN PART, 2026-09-01 — read K25/K26 first.** §5.0.1 describes the **level 1**
> fix (assert the z-order after the show/hide). 👤 The owner's acceptance run showed it left a
> visible ~50–230 ms flicker, and correctly named it a symptom fix. The phase then took the
> **root** fix instead: overlay ownership follows the requesting window for as long as the
> overlay is on screen. Both level-1 helpers are **deleted**. What remains true in §5.0.1 is the
> *reasoning* — including why re-owning was initially rejected (K12) and how that objection is
> answered by making ownership temporary.
