...
**Status:** ✅ **DONE on Windows** — landed 2026-09-15 as `57812cf` (extraction) + `a91a34a` (fix), every row GREEN with its RED observed. `P10a-A6` was downgraded by the adversarial panel on 2026-09-15 and re-closed 2026-09-16 by `07c69de` (the basket-insertion ownership check). ⚠️ This line read *"NOT STARTED"* until 2026-09-16 — it was never updated when the phase landed, which is its own small lesson about status lines nobody reads. Owed: `P10a-A5`'s poller half (`PAYMENT_TEST_BATCH.md` M9) and the `F3-10a` dedupe question (10e).
...
| `P10a-A6` | `internalize_action`: subject mismatch ⇒ error; nothing credited ⇒ non-200 | pre-fix: 200 with `total_received == 0` | the HTTP status and body, not the log | T1/T2 | ✅ **GREEN, RED seen — re-closed 2026-09-16.** Downgraded by the panel on 2026-09-15 because the probe drove four envelope shapes and never a `basket insertion` output spec, which was ungated. Now fixed (`07c69de`) and measured with the arm the old probe never drove (`scratchpad/p10e_basket_probe.py`): 🔴 **RED** pre-fix, a real mined transaction belonging to a stranger ⇒ **HTTP 200** and balance **29,077,178 → 128,714,797 (+99,637,619 sats)**, stored `spendable=1 confirmed=1`. 🟢 **GREEN** post-fix, same probe ⇒ **400 `ERR_NO_OUTPUTS_OWNED`**, zero rows added, balance unchanged. ⛔ Note for anyone re-running it: output **0** of that transaction is worth 0 satoshis and returns 400 *while still writing the row* — only the **value-bearing** output exposes the 200. A probe reading status codes alone calls this healthy, which is exactly how it survived. Earlier evidence stands: pre-fix stranger's tx ⇒ 200 `unconfirmed`; mismatch ⇒ attacker bytes sent to WoC |
