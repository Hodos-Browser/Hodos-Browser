...
| D10 | 🍎 **`P10b-A5` queue half — two queued payment prompts, macOS** | The second modal appears on its own after the first is answered, each showing **its own** amount, with the **"1 of N"** line. ⭐ **Costs ZERO satoshis: answer Deny on both** — the approval gate runs before the spend, so the queue is exercised without moving money (the Windows `W6` run spent real money only because it clicked Approve). 📏 Already measured here 2026-09-19: the modal's **rendering** is green and the **"1 of N" line now appears** (10e `0bc64d4` fixed the frozen `queuedFromSite` that made Windows' `W6` miss it); what is owed is `PendingRequestManager` **sequencing**, which needs two genuinely queued requests — this dev wallet's balance is 0 | `MAC_RELAY_BETA3.md` round 2026-09-19d; `10b-one-click-one-spend/PHASE_CONTRACT.md` §4c |
