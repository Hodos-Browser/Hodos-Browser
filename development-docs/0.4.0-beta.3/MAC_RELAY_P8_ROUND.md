...
| 🍎 **Phase 8d stages 2–3 (2026-09-14) — backend supervision. THIS ONE HAS A REAL macOS HALF, and it is yours.** Windows: `cef_browser_shell.cpp` gained `BackendSupervisorLoop` (detached `std::thread`, 2 s; child death = handle signalled; bounded relaunch 3× 2/4/8 s; `invalidateWalletStatusCache()` on death; `g_walletServerRunning` no longer forced true on the unhealthy startup path — **your honest-flag shape, now matched**), `RequestWalletRestart()`, `HODOS_NO_SUPERVISE=1` seam; adblock on the same watcher, restart-only | ⚠️ **`cef_browser_shell_mac.mm` touched from Windows:** a 3-line **logging stub** `RequestWalletRestart()` next to `SpawnAdblockServer()` so the shared `wallet_restart` arm in `simple_handler.cpp` links. Also `HttpRequestInterceptor.cpp` (+`invalidateWalletStatusCache`), `simple_handler.cpp` (+`wallet_restart` arm), `cef_browser_shell.cpp` (Windows-only) | **Build, then port:** replace the stub with the real thing — a GCD/`std::thread` loop: `waitpid(g_wallet_server_pid, &st, WNOHANG)` ⇒ dead → `SpawnWalletServer()` with the same 3× 2/4/8 s bound, `g_walletServerRunning=false` + `invalidateWalletStatusCache()` on death, set true when `QuickHealthCheck()` answers; `RequestWalletRestart()` resets the counter and relaunches now; same for `g_adblock_server_pid` restart-only. Rows `P8d-A4` (kill `hodos-wallet` **by path**, expect relaunch ≤ 5 s) and `A5` (exe renamed away ⇒ exactly 3 attempts) are your evidence; the contract §4b has the Windows numbers to match |
| **Phase 8d stage 1 (2026-09-14) — truth about a dead wallet service:** `WalletStatusCache::status()` (three-way), `WALLET_UNAVAILABLE` at the IPC / BRC-100 / BRC-121 gates, `wallet_unavailable` notification; `wallet_status_check` reply gains `serviceReachable` | `HttpRequestInterceptor.cpp` (shared) · `simple_handler.cpp` (shared, `wallet_status_check` arm) — plus React `WalletPanelPage.tsx`, `BRC100AuthOverlayRoot.tsx`, `hodosBrowser.d.ts` | Build only. Free check: stop the dev wallet (by path), open the wallet panel — expect *"Wallet service not running"* + Try again, **not** Create / Recover. 🍎 **Stage 2 (supervision) will have a real macOS half** — `cef_browser_shell_mac.mm :: SpawnWalletServer` / `g_wallet_server_pid`; it is written up in `phase-8d-wallet-supervision/PHASE_CONTRACT.md` and will get its own relay item |
| **O2 + O5 + O11 + O12 (2026-09-14):** `send_transaction` runs its wallet call **off the UI thread** on a dedicated `CefThread` named `hodos-wallet-send` (`cef_thread.h`, `stoppable=false`, created lazily on TID_UI — 📏 all three `TID_FILE_*` ids turned out to be ONE shared thread, and a slow send stalled balance polls behind it); the render handler's deadline task now returns early when the call was already answered; the full-transaction-JSON debug line is gone; a rig-only `HODOS_BRIDGE_REPLY_DELAY_MS` seam (read once in the browser process) holds the send reply N ms. The render-side bridge deadline moved from a file-local `30000` to `kBridgeCallTimeoutMs = 45000` in `WalletService.h`, `static_assert`ed above `kWalletBroadcastTimeoutMs + 5000` — they were **equal** before | `simple_handler.cpp` (shared, `send_transaction` arm only) · `simple_render_process_handler.cpp` (shared: one `#include`, one deleted constant) · `WalletService.h` (shared header — `WalletService_mac.cpp` includes it; nothing in the `.cpp` changes) | Build only. If you want a free check: stop the dev wallet, call `hodosBrowser.wallet.sendTransaction({recipient:'x',amount:1})` from CDP with a `/json/list` probe running — the probe must stay fast (Windows RED on the old binary: **2,061 ms**, one sample; GREEN: see `P8c-A9`) |
