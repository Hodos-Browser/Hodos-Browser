...
| **11 close / W5 sitting -> Phase 12** | **2026-09-19** | ⛔ **subject untouched** by the two Windows changes (a Rust sweeper verdict + a frontend count ref); ⚠️ but macOS landed TWO money-path C++ fixes in this window (`4b5e750`, `8f857d5`) and R-INTEXT was not re-run against them | ⭐ **GREEN, REAL MONEY, FIRST TIME THIS SPRINT** 👤 owner at the keyboard: gold pill on the **originating** tab; 📏 log `OnWalletCallSuccess fired (cefBrowserId=13 -> tabId=2)` = SUBJECT satisfied (addressed by `Tab::id`); chain confirms a real tx + the 1,000-sat treasury fee. ⭐ Free discriminator: 2 createAction attempts, **1** pill -- the failed call fired none. ⛔ **Owed:** the stub-the-emit RED (build-side) and the **BRC-121 paid-retry** emit route | ⛔ not touched | ⛔ not re-run | ⛔ **not run** -- session cap $10 vs a ~$5 dev wallet; 👤 owner will run it at real sites | ⛔ not touched |
| 🚦 **12/13 close -> RELEASE TAG `v0.4.0-beta.3`** | **2026-09-23** | ⛔ **subject untouched, and VERIFIED not assumed** — `git diff 8547c77..HEAD` over `HttpRequestInterceptor.cpp` / `simple_handler.cpp` / `simple_render_process_handler.cpp` matches `IsInternalOrigin|X-Requesting-Domain|GetURL|requesting_domain|OriginFromUrl|IsWalletOrigin` on **one** line, and it is P12's adblock `OnLoadStart` re-push (a scriptlet path), **not** the `wallet_call` origin derivation. No header stamping, no origin derivation, no gate predicate | ⭐ **GREEN, real money, 2026-09-19** (carried — nothing since touches the emit site). ⛔ Still owed: the stub-the-emit RED and the BRC-121 paid-retry route | ⛔ not touched since the 11 close | 🟢 T1 — engine suite green incl. **5 new level-0 tests** (`matrix_c.rs`), which is the one gating change in this window · ⬜ T2 e2e | ⛔ needs a payment | 🔴 **THIS IS WHERE R-UPDATE COMES DUE.** "real N−1→N owed at RC" at every boundary since 2026-08-18 — this is the RC. ⭐ And the N−1 is **`0.3.0-beta.29`**, not beta.2: beta.1/beta.2 were built and **never published** | `preflight -Full` **PASS, all checks ran** · shell compiles · 381 C++ tests pass / 1 skipped |
