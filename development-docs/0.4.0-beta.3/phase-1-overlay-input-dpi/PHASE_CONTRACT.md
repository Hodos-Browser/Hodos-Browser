...
|---|---|---|---|---|---|
| `P1-M10` | 🚨 **COUNTEREXAMPLE to `P1-M1` — the coordinate defect DOES occur on macOS, found by the owner using the app.** `ShowProfilePanelOverlayMacOS` resized the profile panel's NSWindow to a hardcoded **300x400** while `CreateProfilePanelOverlayMacOS` built a **380x520** CEF view. So the FIRST open was correct and every RE-open was **26.7% out horizontally, 30% vertically** — ~120px of drift by the bottom of the panel. Owner's report: *"I click Edit and it opens the profile instead."* Same user-visible failure as the Windows defect, different mechanism: not physical-vs-logical DPI, but the window and the view disagreeing outright. | **MEASURED by differential CGWindowList capture:** with the panel hidden, two 1440-wide windows; opening it added a **300x400** window (num 3414) whose DOM reported **380x520**. Calibration factor verified as exactly 1.0000 against the header (CSS 1440 = window 1440), so the two are directly comparable. AFTER the fix, same procedure: first open **380x520 / 380x520**, re-open **380x520 / 380x520**. Sibling panels checked — cookie (400x500) and download (400x500) match in both paths, so this was single-site drift, not a pattern. Fixed by hoisting `kProfilePanelWidth/Height` so the two paths cannot diverge again. | dev bundle, profile panel overlay, `/profile-picker` | T3 | MEASURED |
| ⚠️ `P1-M1` (amended) | ⚠️ **AMENDED 2026-08-26 — the general claim was too strong; see `P1-M10` for a counterexample.** This row is correct *for the wallet overlay*, which sizes its window dynamically so window and view agree. It is NOT true of every macOS overlay: a panel whose window is resized to different numbers than its view has exactly the Windows failure. What macOS lacks is the *DPI* form of the bug, not the *window-vs-view disagreement* form. As measured for the wallet overlay: no unit gap between window and view — wallet overlay measures NSWindow `visibleFrame(795) − 96 = 699 pt` vs CEF view `400 × 698` css px, `devicePixelRatio = 2` = `backingScaleFactor`. The Windows root cause (physical client vs logical view) does not exist here. | Not run as a click. The CDP-click harness I first wrote is **void** — `Input.dispatchMouseEvent` enters below the native NSView→`CefMouseEvent` layer and would pass with the defect present (HARNESS §6 Q1). A real-`CGEventPost` probe printed four misses **and then failed its own positive control** (a click on the known-good main window also registered nothing); root cause measured — `CGEventPost` is Accessibility-blocked for the session process. Result discarded. | dev bundle, wallet overlay target `127.0.0.1:5137/wallet-panel?iro=0` | T3 | MEASURED (structural) / **NOT RUN** (click) |
