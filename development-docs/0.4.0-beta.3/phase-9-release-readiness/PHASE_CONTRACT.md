...
| `P9-A3` | (D4, if Q1 = yes) right-click on the wallet overlay offers **no** Inspect Element; F12 with the overlay focused opens nothing; right-click / F12 / menu / IPC on a web-page tab all open DevTools | Revert the role gate ⇒ the overlay's context menu shows Inspect Element again and F12 opens DevTools on it | `SimpleHandler::role_` logged at the gate: `wallet` / `brc100auth` refused, `tab_<id>` allowed; a tab navigated to `/settings-page` is **still inspectable** (accepted by design) | T2 (instrumented) + T3 (owner's eyes for the right-click menu) | ✅ 2026-09-14. Instrument: F12 delivered as a **posted `WM_KEYDOWN`** to the HWND (⛔ not SendInput, not CDP — CDP `Input.dispatchKeyEvent` never reaches `OnPreKeyEvent`, measured: 0 log lines, 0 devtools targets on both a tab and the overlay). GREEN (D4 build): F12 → `CEFWalletOverlayWindow` ⇒ `[INFO] DevTools refused on role=wallet (D4: only tab browsers are inspectable)`, nothing opened; F12 → the tab's `Chrome_WidgetWin_1` ("New Tab") ⇒ `DevTools opened via keyboard shortcut` + a top-level window titled `DevTools`. RED (gate stashed, rebuilt, same probe): F12 → wallet overlay ⇒ **opened**, window `DevTools - 127.0.0.1:5137/wallet-panel?iro=50` present — the defect, seen. Then restored + rebuilt + re-probed GREEN. ⏳ T3 half owed to the owner: right-click on the wallet overlay shows no *Inspect Element* (the menu is a native `CefMenuModel`, not observable from CDP); the code path is the same `isTab` branch. Probe: scratchpad `p9_a3_probe.py` |
...
| `P9-D1` | `cargo audit` ×2, `cargo outdated --root-deps-only` ×2, `npm audit` recorded as a hold/bump table in `DEPENDENCY_VERIFICATION.md` with dates and advisory ids; the export-count script prints the three numbers for `cef-binaries/Release/libcef.dll` | Export script pointed at `HodosBrowser.dll` (which statically links our OpenSSL/SQLite) shows a **different** shape — proving the count is of the file named, not a constant; `cargo audit` run against a scratch `Cargo.lock` carrying a known-yanked/advisory crate reports it (the tool is seen to find something) | script output names the file path + size + md5 of the DLL measured; the shipped DLL is the one `release.yml`'s asset asserts (`+g9ccef04+`) | T0/T2 | ✅ 2026-09-14 GREEN: `cargo audit` wallet **6** vulns (3× `rustls-webpki` 0.101.7 name-constraint/CRL, `h2`, `time`, `bytes`) + adblock **2** (`h2`, `time`); `cargo outdated --root-deps-only` 30 / 10 behind; `npm audit` 16 (11 high, all in-range fixes); export script on `libcef.dll` (md5 `8c761ddc…`) = 247 / 240 / 1 (`sqlite3_dbdata_init`), same as 2026-08-17. RED: the script on `HodosBrowser.dll` = 1 / 0 / 0 (different shape ⇒ it measures the named file). ⚠️ The planned `cargo audit` RED against a scratch lockfile was **not run** — the tool found 8 real advisories on the real lockfiles, which is a stronger demonstration that it detects than a seeded one. Table + decisions in `DEPENDENCY_VERIFICATION.md`; **nothing bumped** |
...
## 9. Adversarial review — the four questions (`HARNESS.md` §6), answered by the author against the author

1. **Can I make each test pass with the feature removed?**
   `P9-A1`: no — the RED half is the *same* probe on the same binary reading "nothing bound" (picker), so the
   probe is shown able to see both states; a release-shaped false green is still possible only through the
   dev safeguard's blind spot, which is why **I8** is owed rather than claimed. `P9-A2`: the dev half cannot
   go red without a release run — said so; I8. `P9-A3`: no — the gate was **stashed, rebuilt and the overlay
   opened DevTools** (`DevTools - …/wallet-panel` window seen); the CDP key path that *cannot* reach the
   handler was measured and rejected as an instrument before it could fake a green. `P9-B1`: absent-then-present
   on the remote. `P9-C1`: `--expect-cef` mismatch and a foreign `libcef.dll` both refused before launch; the
   negative-control run inverted correctly. `P9-C2`: the same shell body refused four wrong tokens and — before
   I fixed it — refused the right one too (the `tr` bug), which is the gate seen failing closed. `P9-D1`: the
   export script's numbers moved when pointed at a different DLL; `cargo audit` found eight real advisories,
   so the seeded-lockfile RED was not run and is recorded as such.
2. **What is the subject?** A1/A2/A3: pid + `ExecutablePath` under `cef-nativeuildin\Release`, log file
   `debug_output-<pid>.log`, `/json/version` = `Chrome/150.0.7871.187`; the installed build (pid 34432, 9222)
   was in the process table throughout and never touched. B1: the **remote** ref list, not the clone's. C1:
   `CEF_VERSION` **and** md5 of the loaded `libcef.dll` == staged; `role=tab_1` in the log for the farbled
   host. C2: the step body extracted from `promote.yml` by regex (not retyped), `release.yml` from the same
   tree. D1: file path + size + md5 printed by the script.
3. **What would I expect to see if this were broken, and did anyone look?** Broken D2: 9322 bound with
   `HODOS_DEV` unset — not observable here (I8). Broken D3: harness 403 in dev — looked: attach succeeded with
   two Origins. Broken D4: a `DevTools` window over the wallet panel — looked, on the gate-less build, and saw
   it. Broken B1: `ls-remote` still branch-only — looked. Broken C: the P4e token passing — looked, refused.
   Broken D1: a script that prints constants — looked, it did not.
4. **Measurement or code reading?** Every ✅ cell in §4 is a measurement and says what was run. Code readings
   are labelled: the context-menu half of D4 (owner's eyes), the I8 release halves, and the
   `secp256k1`/RustCrypto "hold" verdicts in the dependency table.

**Boundary regression (`../REGRESSION_SET.md`), honestly:** T1 halves of `R-PERIM` and `R-DUST` ran inside
`preflight -Full` (`cargo test`); `R-UPDATE` stays T1-only (`hodos_tests`; I3 owed as at every boundary).
The T2/T3 halves of `R-INTEXT`, `R-GOLD`, `R-CLOSE`, `R-COUNT`, `R-PERIM` were **not re-run** this boundary
— recorded as **SKIPPED, run INCOMPLETE** per §8, not as pass. Justification for the reader, not a
substitute: this phase's only shipped code paths are the CDP-port block, one command-line switch, and the
DevTools entry points; none is on the wallet-call, payment, overlay-close or session-counter path, and
`preflight -Full`'s `T1c`/`T1d` cover the handler and the frontend build.
