...
| **10 — critical advisories** | `CRITICAL_UPDATES.md` CU-3 (+CU-6) · CU-1 (+CU-8, CU-9) · CU-2, as sub-phases 10a/10b/10c | ⭐ **ADDED 2026-09-15 by owner decision.** Three high-severity money-path defects, all present in the shipped beta.29 by code reading, one of them (CU-3) needing no user action. Gates the release harder than anything visual, so it runs **before** the UI tail. See `phase-10-critical-advisories/README.md` |
