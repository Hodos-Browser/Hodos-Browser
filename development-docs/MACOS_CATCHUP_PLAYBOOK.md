...
#### C13 — Wallet-UI → C++ bridge migration / dev-port deconfliction (P1; Windows LANDED 2026-06-25, commits `a95e01e`/`4ba4fe7`/`3b3e063`/`6331ef1`) — COMPILE-VERIFY + TEST
- **Read first:** `development-docs/0.4.0/WALLET_UI_BRIDGE_MIGRATION.md` (full design, reuse-based, red-teamed `wf_d67e8bd5-37e`). This is NOT a mac-port chunk you implement from scratch — it's **shipped cross-platform work you must compile-verify on mac and test**.
- **What changed & why:** the goal is to let the **installed** browser and the **dev** browser (`HODOS_DEV=1`) run **at the same time**. They previously both used wallet port 31301 / adblock 31302 → conflict. Now dev uses **31401/31402** (env-gated). Two pieces:
  1. **Port gate** (commits 1+2): new `cef-native/include/core/PortConfig.h` — `hodos::WalletPort()/AdblockPort()` return 31401/31402 iff `getenv("HODOS_DEV")=="1"` else 31301/31302 (dev flag cached once via a function-static lambda — C++11, clang-fine); plus `WalletUrl()/AdblockUrl()/WalletBaseUrl()/IsWalletHostPort()`. Mirrors the Rust gate (`rust-wallet/src/main.rs::wallet_port()`, `adblock-engine/src/main.rs::adblock_port()`, and `monitor/task_backup.rs`).
  2. **Bridge** (commits 3+5): the first-party React wallet UI no longer direct-fetches `127.0.0.1:31301`. It calls `window.__hodos_walletCall` (the existing Phase-2.5 `wallet_call` IPC bridge), so **C++ owns the port** and the frontend never knows it. Commit 3 (a) makes `runIpcCallDirect` (`HttpRequestInterceptor.cpp`) omit `X-Requesting-Domain` when `IsInternalOrigin(origin)` — the trust fix — and (b) splits the `__hodos_walletCall` transport IIFE out of `CWIShimScript.h` into `WALLET_CALL_BRIDGE_SCRIPT`, injected on internal+overlay pages (was external-only) in `simple_render_process_handler.cpp`. Commit 5 is the frontend (`walletApi.ts` + 77 fetch sites → `walletFetch`).
- **MAC FILES EDITED but NEVER COMPILED ON MAC (the compile-verify gate):** `cef_browser_shell_mac.mm` (added `#include "include/core/PortConfig.h"`; `QuickHealthCheck`/`QuickAdblockHealthCheck` now use `hodos::WalletUrl("/health")`/`hodos::AdblockUrl("/health")`; the two `SendShutdownRequest(...)` calls use `hodos::WalletPort()`/`hodos::AdblockPort()`) and `cef-native/src/core/WalletService_mac.cpp` (added the PortConfig include; `baseUrl_(hodos::WalletBaseUrl())`). The shared files (`PortConfig.h`, `HttpRequestInterceptor.cpp`, `simple_render_process_handler.cpp`, `CWIShimScript.h`, `AdblockCache.h`) compile on Windows but their mac compile is also first-time here. **grep, don't trust these line refs** — symbols are stable, lines will drift.
- **Mac action:** (a) **compile-verify** — clean mac build; confirm `PortConfig.h` resolves from both mac include roots (`include/core/...` in `_mac.mm`, `../../include/core/...` in `WalletService_mac.cpp`), no dangling symbol. (b) **Functional test — the whole point:** with the Rust wallet + adblock running under `HODOS_DEV=1` (dev launchers already set it), confirm the **dev** browser talks to **31401/31402** and uses data dir `HodosBrowserDev`, while a separately-running **installed/prod** instance keeps **31301/31302** + `HodosBrowser` — **both at once, no cross-talk.** Then normal wallet smoke (create/load/balance/send/approved-sites/settings) through the bridge.
- **SECURITY test (do this — red-team SEV-1):** from an external https dApp page, `window.__hodos_walletCall('export','/wallet/export',{password:'x'},'POST')` and `...'/wallet/backup'...` must be **rejected** (Rust 403 — the export/import `X-Requesting-Domain` guard added in commit 1 mirrors `wallet_restore`); first-party UI export/backup must still **work** (no header). Confirm Rust logs show **zero** `X-Requesting-Domain` on first-party `wallet_call`s.
- **Commit 4 (large-response chunking) — IMPLEMENTED `1922aa1` + adversarially reviewed (3 agents). COMPILE-VERIFY + LIVE-TEST, do NOT re-implement.** `sendWalletResponseIpc` (`HttpRequestInterceptor.cpp`) size-gates at 256 KB: ≤256 KB takes the original single-message `wallet_response` path **byte-identically**; >256 KB is split into UTF-8-boundary-safe chunks → new `wallet_response_chunk` render handler (`simple_render_process_handler.cpp`) → JS `window.__hodos_walletResponseChunk` reassembler in `WALLET_CALL_BRIDGE_SCRIPT` (`CWIShimScript.h`) that resolves only on complete + **fail-closed** byte-length match, else rejects (no silent truncation). All cross-platform (shared files) — your job is to (a) confirm it compiles on mac and (b) **live-test the big-payload path:** export a large/active wallet through the UI and confirm it round-trips (export → re-import → balances match). NOTE the backup/export UI button is **not wired yet** (owner deferred), so you may need to hit `/wallet/export` directly to exercise the chunked path. It's the highest-blast-radius shared path (every wallet response flows through `sendWalletResponseIpc`), so test the normal small-payload calls too.
- **Effort:** S (compile-verify) + S (test). **Risk:** Low for the compile-verify; the bridge logic is shared/already exercised on Windows. The only real risk is the Commit-4 large-export gap above.

#### C14 — Window + single-instance deconfliction (P1; Windows LANDED 2026-06-25, commits `1aeaedd`/`f9408fd`) — COMPILE-VERIFY + one REQUIRED mac Info.plist change
- **Read first:** `development-docs/0.4.0/WINDOW_INSTANCE_DECONFLICTION.md` (adversarially reviewed, GO-WITH-EDITS).
- **What/why:** running the dev build + installed build at once (the port-deconfliction goal) collided at the **Windows window layer** — clicking wallet in one instance focused the OTHER instance's overlay. Windows root causes, all fixed Windows-side: `FindWindow` (system-wide) was used to locate the app's OWN overlays → replaced with the in-process `g_*_overlay_hwnd` globals (+ `IsWindow()` guards); the single-instance **pipe**, **AUMID**, and **DevTools `remote_debugging_port`** were keyed on profile only (dev+prod both `Default`) → now gated on `hodos::IsDevEnv()` (pipe gets a `dev.` marker, dev AUMID `HodosBrowser.Dev`, dev DevTools port `+100`).
- **Mac CODE: compile-verify only.** Both commits are `#ifdef _WIN32`. The FindWindow change lives in cross-platform `simple_handler.cpp` but inside `_WIN32` blocks; the mac `#elif __APPLE__` arms already use in-process `extern NSWindow* g_*_overlay_window` (and null them on close), UNCHANGED. Confirm it compiles; no mac behavior change.
- **⚠️ REQUIRED mac change (the mac EQUIVALENT of the fix — NOT a nicety):** macOS has **no app-level single-instance** (`SingleInstance.cpp` non-Windows branch is a stub; only `flock` per data-root protects the DB). Two builds packaged as `.app`s sharing `CFBundleIdentifier` (`com.hodosbrowser.app`, hardcoded `Info.plist:9-10`) are treated as the **SAME app** by LaunchServices → `applicationShouldHandleReopen:` → `[NSApp activateIgnoringOtherApps:YES]` (`cef_browser_shell_mac.mm` ~164/205) can raise the **wrong** instance = the mac cross-focus. **Data-root + flock covers DB corruption, NOT activation identity.** Give the dev `.app` a distinct `CFBundleIdentifier` (e.g. `com.hodosbrowser.app.dev`) + display name ("Hodos Browser (Dev)"). **Info.plist is DevOps-owned (§4-D) → surface to owner; preserve the `SU*`/no-silent-update keys exactly.** Moot ONLY when the mac dev build runs **directly** via `mac_build_run.sh` (not as a packaged `.app`).
- **Test (mac):** run dev `.app` + installed `.app` together → click wallet/settings/backup in each stays in its OWN instance; Dock shows distinct apps; a 2nd launch of each forwards to the right instance. If they cross-activate, it's the bundle-ID issue above.
- **Effort:** XS (compile-verify) + S (Info.plist + test). **Risk:** Low (Windows code is inert on mac; the bundle-ID change is the only real mac action).
