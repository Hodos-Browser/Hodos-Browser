...
- **⚠️ KNOWN GAP — Commit 4 (large-export chunking) NOT DONE (deferred, needs live test):** first-party `/wallet/export` now returns its body through the **shared `wallet_response` render path** — a single `frame->ExecuteJavaScript(...)` with the whole payload escaped into the JS source (`simple_render_process_handler.cpp`, the `wallet_response` handler ~the `__hodos_walletResponse(...)` call). For a typical wallet this is fine; for a **very large export** (tens of MB — active wallet with thousands of txs/outputs) the multi-MB JS-source compile on the render thread may hitch or the CEF process-message may exceed limits and the export **fails (detectably — re-import errors; NOT silent corruption)**. **Test it on mac too:** export a large/active wallet through the UI and confirm it round-trips (export → re-import → balances match). If it fails, that's the unbuilt Commit 4 (size-gated chunked delivery — see plan §4 Rule 3 / §0). Coordinate with the owner before implementing; it touches the highest-blast-radius shared path.
