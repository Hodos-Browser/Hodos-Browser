# beta.4 sprint plan — sprint level only

**Written:** 2026-08-29, telescope pass. **Companion:** `TELESCOPE.md` (how these interact),
`README.md` (scope and settled decisions).

⛔ **This document stops at sub-sprint granularity by design.** No APIs, no schemas, no phase
...
**Order — settled:** **0 `reqwest` TLS bump (👤 added 2026-09-15, first)** → **0.5 UTXO reservation ownership** (👤 placed 2026-09-15, owner to confirm at kickoff; `../TICKET_reservation_ownership_converge_on_spent_by.md`) → 1 Guard → 2 1Sat → 3 OpNS → 4 Backup → tickets. Reasoning in `README.md`; sprint 0's outline and research notes in `sprint-0-reqwest-tls-bump/README.md` — it needs its own scoping pass before any code.
...
Sprint 1 implements the 1-sat/inscription classifier only. BSV-20/21 and future protocols arrive
...
(`output_repo.rs:98,148`). This sprint fills the classification gap on ingest; it does not build a
second exclusion system.

### Sub-sprints

| # | Sub-sprint | What it produces |
|---|---|---|
...
| Recovery/sweep/reconcile parity | Ordinal transfer, listing, or purchase — that is sprint 2 |
| The standing "no automatic path spends an unclassified output" invariant | Permission classes for token spends — sprint 2 |
...
**Prerequisite:** sprint 1. Ordinals are the assets sprint 1's guard protects; shipping them first
means shipping a feature and its destroyer together.

**Carried in:** `sprint-2-1sat-ordinals/README.md` and `RESEARCH_FINDINGS.md`. ⚠️ Both predate the
...
| # | Sub-sprint | What it produces |
|---|---|---|
| 2.1 | **BRC-147 basket + BRC-165 normalization** | Ordinals land in the `1sat` basket by the normalization rules, not by ad-hoc naming. Closes sprint 1's classifier against the real spec. |
| 2.2 | **Provenance — BRC-150** | Provenance rows carried and verified. ⚠️ These rows (`beefB64`) are the workload sprint 4 measures against; **record their real sizes as they land.** |
...
| Recording real provenance-row sizes for sprint 4 | Naming — that is sprint 3, built on the same outputs |
...
**Prerequisite:** sprint 2. A name is carried by a 1-sat output; names inherit ordinal handling.
...
**A new naming sprint doc is required** — `sprint-3-opns-naming/` currently holds scope only.
...
| # | Sub-sprint | What it produces |
|---|---|---|
| 3.1 | **Outline against BRC-174** | The new sprint doc. Reads the old naming folder once, extracts what survives, and states what BRC-174 changed. |
...
- Is 3.4 in the **release**, or a parallel public artifact on its own timeline? (Also in
  `README.md`'s decisions-owed list — it changes the sprint's size materially.)
...
**Prerequisite:** sprints 2 and 3 — for the workload, not for the code. Deltas exist to solve
token-heavy wallets, and BRC-150 provenance rows are that workload.

⛔ **`sprint-4-onchain-backup-sync/IMPLEMENTATION_PLAN.md` stands and is NOT to be redesigned.**
8 phases, decisions D1–D15, two adversarial reviews, owner sign-offs recorded. The microscope pass
for this sprint **reconciles** that plan against what sprints 1–3 actually built. It does not rewrite
it. If a sprint 1–3 outcome breaks a D-decision, that is a finding to surface, not a licence to
re-plan.

### The sprint's own open question — the owner's, answered by measurement
...
harness §8 exists to catch. Sprint 2's provenance rows are the measurement subject — which is why
...
| Reconciling the plan against sprints 1–3 outcomes | Anything the plan already marked out of scope |

### Owed to microscope

- Which of D1–D15 the token work touches, and whether any is invalidated by sprints 1–3.
- The measured answer to the backup-vs-refetch question, with the experiment named.
- How a classified-but-unrecoverable output behaves on restore — the intersection of sprint 1's
  fail-closed rule and sprint 4's recovery path. **This is the sharpest cross-sprint edge in the
...
`tickets/` — a folder the **owner reviews and assigns into sprints**. Not beta.3's flat
...
- A ticket is not work until it carries a sprint assignment.

---

## Cross-sprint dependencies at a glance
...
```
       ...
       └───────────────────────────┬──────────────────────────────┘
                                   ...
                                   ▼
  S1 Guard ──── classification seam ────► S2 1Sat ──── 1-sat outputs ────► S3 OpNS
     │                                       │                                │
     │ fail-closed rule                      │ beefB64 provenance rows        │ name outputs
     └───────────────────┬───────────────────┴────────────────────────────────┘
                         ▼
                    S4 Backup  (measures the workload S2/S3 created;
                                inherits S1's fail-closed rule on restore)
```

**The three edges that matter**, in the order they will bite:

1. **S1 → S2.** The classifier sprint 1 writes must be the one BRC-147/165 normalization expects. If
   sprint 1 invents its own basket naming, sprint 2 rewrites it.
2. **S2 → S4.** Sprint 4's central question is answered with sprint 2's data. If sprint 2 does not
   record provenance-row sizes as it goes, sprint 4 measures retrospectively or guesses.
3. **S1 → S4.** Fail-closed on restore. An output that cannot be classified during recovery must not
   silently become spendable — and must not silently vanish either. Neither sprint owns this edge
