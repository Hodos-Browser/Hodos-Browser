...
| `BRC100Bridge.cpp` | BRC-100 protocol HTTP bridge to Rust wallet. Windows uses WinHTTP, macOS uses libcurl. 15 API methods: `getStatus`, `isAvailable`, `generateIdentity`, `validateIdentity`, `createSelectiveDisclosure`, `generateChallenge`, `authenticate`, `createSession`, `validateSession`, `revokeSession`, `createBEEF`, `verifyBEEF`, `broadcastBEEF`, `verifySPV`, `createSPVProof`. WebSocket stubs (placeholder). |
| `BRC100Handler.cpp` | V8 handler for `window.hodosBrowser.brc100.*`. Implements `CefV8Handler::Execute()` dispatching 15 methods. `RegisterBRC100API()` wires up the V8 object tree. Contains `V8ValueToJSON()` and `JSONToV8Value()` conversion helpers. Delegates all HTTP calls to `BRC100Bridge`. |
