...
| `ManifestFetcher.cpp` | Parses a dApp's wallet manifest into the struct the connect-bundle modal renders. `ParseFromJson(json)` is pure, never throws, and is the **production** path (the interceptor re-parses Rust's embedded bytes with it). `ManifestUrls(origin)` returns both locations — BRC-73's `/manifest.json` first, then our legacy `/.well-known/wallet-manifest.json` — and rejects anything that is not a bare authority (`R-PATH`). `Fetch(origin)` uses `SyncHttpClient` (3 s, 64 KB) but is **not** on the production path; Rust owns the fetch since 2.6-G.2. 🚨 `valid == true` now means *declared at least one recognised permission*, not *parsed OK* — see beta.3 Phase 0.8. ⚠️ Mirror of `rust-wallet/src/manifest.rs`; **change both or neither**. Structs in `ManifestFetcher.h`. Consumed by `openManifestConnectBundleModal`. |
...
| `WalletService.cpp` | **Windows.** Wallet HTTP client over WinHTTP, base URL from `hodos::WalletBaseUrl()`. Methods per `include/core/WalletService.h`: `ensureInitialized`, `isHealthy`, `getWalletStatus`, `getWalletInfo`, `markWalletBackedUp`, `generateAddress`, `sendTransaction`, `getBalance`, `isConnected`, `setBaseUrl`, `makeHttpRequestPublic`, plus daemon lifecycle (`createWallet`, `loadWallet`, `getAllAddresses`, `getCurrentAddress`, `createTransaction`, `signTransaction`, `broadcastTransaction`, `getTransactionHistory` were removed in beta.3 Phase 8c batch 2 with the orphaned IPC handlers that were their only callers) (`startDaemon`, `stopDaemon`, `isDaemonRunning`, `setDaemonPath`, `monitorDaemon`). |
...
| `BookmarkManager.cpp` | Bookmark CRUD with SQLite (`bookmarks.db`). Singleton. Tables: `bookmark_folders`, `bookmarks`, `bookmark_tags`. `AddBookmark`, `CreateFolder`, `GetAllBookmarks`, `SearchBookmarks`, `DeleteBookmark`, `MoveBookmark`, etc. Initialized with the profile-specific `user_data_path`. ⚠️ Since beta.3 Phase 8c batch 4 (2026-09-13) only `AddBookmark`, `RemoveBookmark`, `SearchBookmarks`, `GetAllBookmarks`, `IsBookmarked` have an IPC caller; the folder and tag methods (`CreateFolder`, `ListFolders`, `UpdateFolder`, `RemoveFolder`, `GetFolderTree`, `GetAllTags`, `GetBookmark`, `UpdateBookmark`) are orphaned — kept, reported, not deleted |
| `CookieManager.cpp` | CEF cookie CRUD via `CefCookieManager`. Every `Handle*` takes the Phase 8c bridge `requestId` and replies `[requestId, json]`; the empty-jar case (CEF never calls the visitor) is answered from `CookieCollector`'s destructor. Static methods: `HandleGetAllCookies`, `HandleDeleteCookie`, `HandleDeleteDomainCookies`, `HandleDeleteAllCookies`, `HandleClearCache`, `HandleGetCacheSize`. Uses `CookieCollector` (`CefCookieVisitor`), posts results back to the UI thread via `SendResponseTask`; `CacheSizeTask` runs on `TID_FILE_USER_BLOCKING`. |
