...
**151 live message names** dispatched in `SimpleHandler::OnProcessMessageReceived()`. Two additional names — `overlay_hide_NEVER_CALLED_12345` and `overlay_hide_NEVER_CALLED_67890` — are guarded by `if (false && …)` and are dead code; they are **not** counted below.
...
| Wallet operations | `wallet_call` (Phase 2.5 IPC bridge), `wallet_status_check`, `get_balance`, `send_transaction`, `address_generate`, `wallet_prevent_close`, `wallet_allow_close`, `wallet_delete_cancel`, `wallet_payment_dismissed`, `open_wallet_permissions`, `wallet_restart` (Phase 8d — asks the shell's backend supervisor to relaunch the wallet child; no reply) | 11 |

> ⛔ Deleted in beta.3 Phase 8c batch 2 (no JS sender or no reachable caller): `create_wallet`, `load_wallet`, `get_addresses`, `get_all_addresses`, `get_current_address`, and the whole former "Transactions" row — `create_transaction`, `sign_transaction`, `broadcast_transaction`, `get_transaction_history`. Deleted with the backup overlay (8c O8, 2026-09-12): `get_wallet_info`, `mark_wallet_backed_up`, `get_backup_modal_state`, `set_backup_modal_state`, `overlay_show_backup`. The bridge messages — `wallet_status_check`, `get_balance`, `send_transaction`, `address_generate`, since batch 3 the six **Cookie management** and nine **Cookie blocking** messages below, since batch 4 the five surviving **Bookmarks** messages, and since batch 5 the six **Ad blocking** toggles/counts, `cookie_check_site_allowed` and `fingerprint_get_site_enabled` (not `fingerprint_set_site_enabled`, which has no reply), and since batch 6 `paid_cache_get_size` / `paid_cache_clear` — carry a **request id in arg 0** and reply through `ResolveBridgeCall` / `RejectBridgeCall` in the render process rather than a `window.on*` global; `address_generate` also runs its wallet call off the UI thread (8c O9), and since 8c O5 (2026-09-14) so does `send_transaction` — on a **dedicated `CefThread`** (`hodos-wallet-send`, 8c O12), because ⛔ **all three `TID_FILE_*` ids are one shared thread in this process** (libcef shared single-thread runners; `USER_VISIBLE` and `USER_BLOCKING` are both Chromium's FOREGROUND environment) and a 30 s broadcast on it stalled every balance poll behind it. It used to hold the UI thread for the whole 30 s broadcast timeout, and its bridge deadline (30 s) was **equal** to that timeout. The deadline is now `kBridgeCallTimeoutMs` = **45 s**, owned in `WalletService.h` beside the broadcast timeout with a `static_assert` that it clears it. `HODOS_BRIDGE_REPLY_DELAY_MS` (rig-only, read once in the browser process, same family as `HODOS_WALLET_SYNC_UI`) holds the send reply for N ms so a late reply can be produced without money. Since Phase 8d stage 1 (2026-09-14) `wallet_status_check` replies carry **`serviceReachable`** — `false` when the wallet never answered — so `exists:false` is no longer the answer for a dead service.
...
2. **Trusted overlay wallet bypass** — `hodos::IsOurWalletOrigin(url)` from roles `wallet` / `wallet_panel` / `settings` → `nullptr` (native CEF handling; avoids `CefURLRequest` forwarding issues on macOS).
...
// Only on overlays with text input (wallet, settings, brc100 auth,
...
**Full-window overlays** (BRC-100 Auth, Notification): sized from `GetWindowRect(g_hwnd)`, cover the entire main window. (The Backup modal overlay was deleted on Windows in beta.3 Phase 8c O8; its macOS creation function remains until Mac's relay round removes it.)
