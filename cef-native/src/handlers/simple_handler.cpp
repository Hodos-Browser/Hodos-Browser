...
void SimpleHandler::OnLoadingStateChange(CefRefPtr<CefBrowser> browser,
    ...
    // setSiteInfoContext hasn't run yet — Vite loads ~17 MUI icon ESM modules
    // AFTER OnLoadingStateChange fires, so React mounts ~400-500ms later.
    // Retry at 300ms and 600ms to cover the full Vite module waterfall.
    if (!isLoading && role_ == "siteinfopanel" &&
        ...
        if (frame && frame->IsValid()) {
            ...
            for (int delay : {300, 600}) {
                ...
                }, retry_browser, retry_host, retry_security), delay);
            }
        }
    }
...
bool SimpleHandler::OnProcessMessageReceived(
    ...
    if (message_name == "bookmark_add") {
        // Phase 8c batch 4 — MIGRATED. Arg 0 is the bridge request id; payload args shifted to 1..
        const int reqId = message->GetArgumentList()->GetInt(0);
        CefRefPtr<CefListValue> args = message->GetArgumentList();
        std::string url = args->GetString(1).ToString();
        std::string title = args->GetString(2).ToString();

        // Parse folder_id: empty string means -1 (root)
        int folder_id = -1;
        if (args->GetSize() > 3) {
            std::string folderStr = args->GetString(3).ToString();
            if (!folderStr.empty()) {
        ...
        if (args->GetSize() > 4) {
            std::string tagsStr = args->GetString(4).ToString();
            if (!tagsStr.empty()) {
                try {
    ...
    if (message_name == "bookmark_remove") {
        ...
        CefRefPtr<CefListValue> args = message->GetArgumentList();
        int64_t id = 0;
        try { id = std::stoi(args->GetString(1).ToString()); } catch (...) {}
    ...
    if (message_name == "bookmark_search") {
        ...
        const int reqId = message->GetArgumentList()->GetInt(0);
        CefRefPtr<CefListValue> args = message->GetArgumentList();
        std::string query = args->GetString(1).ToString();
        int limit = 50;
        int offset = 0;
        if (args->GetSize() > 2) {
            try { limit = std::stoi(args->GetString(2).ToString()); } catch (...) {}
        }
        if (args->GetSize() > 3) {
            try { offset = std::stoi(args->GetString(3).ToString()); } catch (...) {}
        }
    ...
    if (message_name == "bookmark_get_all") {
        ...
        int offset = 0;
        if (args->GetSize() > 1) {
            std::string folderStr = args->GetString(1).ToString();
            if (!folderStr.empty()) {
                try { folder_id = std::stoi(folderStr); } catch (...) { folder_id = -1; }
            }
        }
