...
> **Vestigial state:** `simple_handler.h` still declares 15 `static CefRefPtr<CefBrowser>` members (`webview_browser_`, `header_browser_`, `wallet_panel_browser_`, `overlay_browser_`, `settings_browser_`, `wallet_browser_`, `backup_browser_`, `brc100_auth_browser_`, `notification_browser_`, `settings_menu_browser_`, `omnibox_browser_`, `cookie_panel_browser_`, `download_panel_browser_`, `profile_panel_browser_`, `menu_browser_`). They are defined to `nullptr` in `simple_handler.cpp` and cleared in the overlay-close paths, but the accessors above no longer read them. ⛔ `header_browser_` was **deleted** in beta.3 Phase 11 item 8 (2026-09-18): its one remaining read was a null-check in `OnProcessMessageReceived()` that could never be true, and the broadcast it guarded — *"so the header picks up changes (e.g. search engine change made in settings tab)"* — had therefore never fired, so changing the search engine did nothing until a restart. Replaced by `BroadcastSettingsChanged()`, which walks every window. 14 statics remain. Do not treat these as the source of truth for "which browser is which".
