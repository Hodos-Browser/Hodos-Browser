...
| `FingerprintScript.h` | `FINGERPRINT_PROTECTION_SCRIPT` — embedded JS constant injected via `OnContextCreated`. Mulberry32 PRNG seeded per-domain; farbles Canvas (`getImageData`, `toDataURL`, `toBlob` — `:34–71`), WebGL **`readPixels` only** (`:77–97`), AudioContext (`getChannelData`, `getFloatFrequencyData` — `:137–158`). Also carries two **anti-bot** surfaces that are *not* farbling: the 5-entry `navigator.plugins` spoof (`:99–126`) and `navigator.webdriver = false` (`:128–133`) — both must be re-homed before the block is deleted (BOT-1). **Does NOT farble** WebGL `getParameter`/vendor/renderer, `hardwareConcurrency`, `deviceMemory`, or screen resolution — all four were deliberately dropped as detectable/low-entropy, per the header's own comments at `:11–15`. *(Corrected 2026-08-05: this row previously claimed `getParameter`, `hardwareConcurrency` and `deviceMemory` were farbled. They are not, and never were in this file.)* |
