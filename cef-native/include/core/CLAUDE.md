...
| `FingerprintProtection.h` | `FingerprintProtection` singleton: per-session 32-byte CSPRNG token (Windows CryptGenRandom / macOS SecRandomCopyBytes), per-domain seed via hash mixing (`GetDomainSeed`), global toggle (`IsEnabled`/`SetEnabled`), **per-site toggle persisted to `fingerprint_settings.json`** (`IsSiteEnabled`/`SetSiteEnabled`/`LoadSiteSettings`/`SaveSiteSettings`), and a hardcoded auth-domain exemption list (`IsAuthDomain()`) |
| `FingerprintScript.h` | `FINGERPRINT_PROTECTION_SCRIPT` — embedded JS constant injected via `OnContextCreated`. Mulberry32 PRNG seeded per-domain; farbles WebGL **`readPixels` only** and AudioContext (`getChannelData`, `getFloatFrequencyData`). Each remaining fragment is the P4b/P4c teardown unit — deleted in the same commit its native Blink replacement lands (atomic per-value rule I-4). **Canvas farbling was DELETED 2026-08-06 (P4a / C3)** — `getImageData`/`toDataURL`/`toBlob` are now farbled natively in Blink by the `hodos_farble_canvas2d` fork patch, at API-call time, so they also report `[native code]` again. Leaving the JS alongside the native patch would have *corrupted* canvases rather than merely double-farbling: the `toDataURL`/`toBlob` overrides farbled via a `getImageData`→`putImageData` round-trip, writing perturbed pixels back into the canvas itself. **Carries no anti-bot surfaces any more:** the 5-entry `navigator.plugins` spoof and the `navigator.webdriver = false` override were **deleted 2026-08-05 (BOT-1)** — measurement showed native Chromium already emits the correct values and both overrides were themselves detectable (our plugin list named the pre-2021 `"Chrome PDF Plugin"`; the `webdriver` redefinition was a prototype-tamper tell). `navigator.webdriver` is now guarded by the TRIPWIRE comment at `cef_browser_shell.cpp`'s `remote_debugging_port` block and asserted by `development-docs/0.4.0/chromium-rebuild/farbling_probe.py`. **Does NOT farble** WebGL `getParameter`/vendor/renderer, `hardwareConcurrency`, `deviceMemory`, or screen resolution — all four were deliberately dropped as detectable/low-entropy, per the header's own comments at `:11–15`. *(Corrected 2026-08-05: this row previously claimed `getParameter`, `hardwareConcurrency` and `deviceMemory` were farbled. They are not, and never were in this file.)* |
...
Three headers ship JS as embedded C string constants, executed via `CefFrame::ExecuteJavaScript` rather than bound as V8 functions:
...
| `FingerprintScript.h` | `FINGERPRINT_PROTECTION_SCRIPT` | `simple_render_process_handler.cpp :: OnContextCreated` |
