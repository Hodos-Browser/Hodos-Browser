...
| `PaidContentCache.h` | `PaidContentCache` singleton: SQLite-backed cache of BRC-121 paid HTTP responses keyed by URL with TTL/expiry from server `Cache-Control: max-age` and 500 MB LRU cap. Struct: `PaidContentEntry`. Read-side handler: `CachedContentResourceHandler.h` |
| `CachedContentResourceHandler.h` | `CachedContentRequestHandler` (CefResourceRequestHandler) + `CachedContentPlaybackHandler` (CefResourceHandler), both header-only. Returned by `SimpleHandler::GetResourceRequestHandler` on cache hit; replays the stored bytes and headers, short-circuiting the entire 402 / Async402 chain |
