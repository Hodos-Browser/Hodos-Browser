...
| `ManifestFetcher.h` | `namespace hodos` — the wallet-manifest parser. `ParseFromJson()` is pure and never throws; `ManifestUrls(origin)` gives both fetch locations (BRC-73 `/manifest.json` first, then legacy `/.well-known/wallet-manifest.json`) and rejects non-authority origins; `Fetch(origin)` wraps `SyncHttpClient` (3 s, 64 KB) but is off the production path. 🚨 `Manifest::valid` means **"declared at least one recognised permission"** since beta.3 Phase 0.8 — ⛔ do not relax it back to "parsed OK", that was the shipped defect. Understands BRC-73 `metanet.groupPermissions`, deprecated `babbage.groupPermissions`, and our top-level `permissions`. `ManifestSpending::monthlySatoshis` (BRC-73) is display-only and must never reach a cap (`R-CAPS`). Parse bounds: `kMaxManifestEntries/Fields/Str/IdStr`. ⚠️ Mirror of `rust-wallet/src/manifest.rs` — change both or neither. Structs: `Manifest`, `ManifestProtocol`, `ManifestBasket`, `ManifestCertificate`, `ManifestSpending`, `ManifestCounterparty`. Unit-tested by `tests/manifest_fetcher_test.cpp`, fixture-driven from `demos/manifest-shapes/` |
...
| `PortConfig.h` | **Single source of truth for backend ports.** `namespace hodos`: `IsDevEnv()` (cached `HODOS_DEV==1`), `WalletPort()` → **31401 dev / 31301 release**, `AdblockPort()` → **31402 dev / 31302 release**, the `*Str()` / `*BaseUrl()` / `WalletUrl(path)` / `AdblockUrl(path)` builders, and the **wallet-traffic predicates** added by beta.3 Phase 5: `AuthoritySpan` / `OriginFromUrl` / `SplitAuthority` (one parse, shared by reader and rewriter), `IsLoopbackHost` (broad: `127.0.0.0/8`, `[::1]`, `localhost`, `*.localhost`), `IsWalletOrigin`, `IsOurWalletOrigin`, `IsMessageboxOrigin`, `IsWellKnownAuthRequest`, `RepointLoopbackToWallet`. ⛔ `IsWalletHostPort` / `IsLoopbackHostPort` are unanchored whole-URL searches kept **only** to back the W3 shadow predicate `LegacyWalletGateMatch`; gate `G12` counts them and beta.4's W8 deletes them. **Never hardcode a port anywhere else** — the dev port must never leak into a release build. Mirrors `rust-wallet/src/main.rs` / `adblock-engine/src/main.rs` |
...
| `Tab.h` | `Tab` struct: per-tab state including `id`, `window_id` (owning BrowserWindow), `title`, `url`, `favicon_url`, `HWND hwnd` (Windows) / `void* view_ptr` (macOS), browser + handler refs, `is_visible`/`is_loading`/`is_closing`/`can_go_back`/`can_go_forward`/`has_cert_error`, `muted`, and `created_at`/`last_accessed` timestamps. ⚠️ `muted` is the user's per-tab audio-mute **intent**, not a mirror of `CefBrowserHost::IsAudioMuted()` — 📏 the CEF mute is per-DOCUMENT and clears on any navigation, so `SimpleHandler::OnLoadingStateChange` re-applies it. Session-lived; not persisted. Two constructors (default, and `(id, url)` which starts `is_loading = true`) |
...
| `WalletService.h` | `WalletService` — HTTP client to the Rust wallet backend: health check, wallet status/info/mark-backed-up, address generation, send + balance, `makeHttpRequestPublic()` for interceptors, and daemon process management (start/stop/monitor). (create/load, address list/current, and the create/sign/broadcast/history transaction methods were removed in beta.3 Phase 8c batch 2 — orphaned.) WinHTTP on Windows (`WalletService.cpp`), libcurl on macOS (`WalletService_mac.cpp`); both resolve the port via `hodos::WalletPort()`. Also owns the per-call transport timeouts (`kWalletDefaultTimeoutMs` 5 s, `kWalletBalanceTimeoutMs` 2 s, `kWalletBroadcastTimeoutMs` 30 s) **and, since 8c O5, the render-side bridge deadline `kBridgeCallTimeoutMs` = 45 s** with a `static_assert` that it clears the broadcast timeout by margin — the two were equal (30 s / 30 s) until 2026-09-14, so a slow-but-successful send could be reported "timed out" |
