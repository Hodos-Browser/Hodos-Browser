...
- `domainWhitelist.json` - Approved domains for BRC-100 requests
...
- ❌ Sequential file writes - potential for corruption
- ❌ No indexing - slow lookups for large datasets
- ❌ No transactions - partial writes possible
- ❌ Memory-intensive - entire files loaded into memory
...
### Phase 1: Database Foundation ✅ (Research Complete)
...
- [ ] Implement table creation/migration functions
- [ ] Create Rust structs matching database tables
- [ ] Implement database connection pool
- [ ] Add database initialization on wallet startup
- [ ] Test schema creation and migrations

### Phase 3: Data Migration
- [ ] Implement JSON → SQLite migration scripts
- [ ] Migrate `wallet.json` → `addresses` table
- [ ] Migrate `actions.json` → `actions`/`transactions` tables
- [ ] Maintain JSON fallback during transition (read both, write DB)
...
- [ ] Implement action storage/retrieval (replace JSON storage)
- [ ] Update wallet handlers to use database instead of JSON
- [ ] Remove JSON file dependencies (keep as backup initially)

### Phase 5: UTXO Management
- [ ] Implement UTXO table operations
- [ ] Create UTXO sync service (fetch from WhatsOnChain)
- [ ] Implement UTXO selection algorithm (Coin Selection)
- [ ] Mark UTXOs as spent when used in transactions
- [ ] Background sync process (every 5 minutes)
- [ ] Detect new incoming UTXOs

### Phase 6: BEEF/SPV Caching
...
1. **Create Database Module**: Set up `rust-wallet/src/database/` structure
2. **Add `rusqlite` Dependency**: Update `Cargo.toml`
3. **Implement Schema**: Create SQL migration files
4. **Start with Core Tables**: Addresses and transactions first
5. **Migrate Existing Data**: Write migration script for JSON → SQLite
6. **Test Thoroughly**: Ensure no data loss during migration
7. **Update Handlers**: Replace JSON storage calls with database calls
8. **Add UTXO Support**: Implement UTXO table and sync service
9. **Add BEEF Caching**: Implement parent transaction and Merkle proof caching
