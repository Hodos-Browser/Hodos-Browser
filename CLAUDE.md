...
| `cef-native/src/handlers/simple_handler.cpp` | CEF client handler (10 interfaces incl. CefDownloadHandler, CefFindHandler); IPC dispatch, keyboard shortcuts (Ctrl+F), context menus (5 context types, all custom `MENU_ID_USER_FIRST` IDs — see working-notes.md #8), download tracking, find-in-page (JS `window.find()` — CEF Find API non-functional in CEF 136). Helpers: `CreateNewTabWithUrl()`, `CopyTextToClipboard()`. Cross-platform wrapped. |
