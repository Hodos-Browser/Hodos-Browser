...
| `manifest.rs` | Wallet-manifest fetch + parse; Rust half of a two-layer parser (`cef-native/src/core/ManifestFetcher.cpp` is the other — **change both or neither**). `manifest_urls()` returns both locations, BRC-73's `/manifest.json` first then our legacy `/.well-known/wallet-manifest.json`; `manifest_url()` keeps the single legacy form. `parse_manifest()` is pure and never panics, and since beta.3 Phase 0.8 returns `Some` **only when at least one recognised permission was declared** — that tightening is what makes the interceptor's existing `domain_approval` fallback fire. Reads **BRC-73** `metanet.groupPermissions`, deprecated `babbage.groupPermissions`, and our own top-level `permissions`, in that precedence. 🚨 `spendingAuthorization.amount` is **monthly satoshis** and lands only in `ManifestSpending::monthly_satoshis` — never in `per_transaction_usd`/`per_session_usd` (`R-CAPS`). Bounded against hostile input (`MAX_ENTRIES`/`MAX_FIELDS`/`MAX_STR`); `R-PATH` rejects any origin that is not a bare authority. Types: `Manifest`, `ManifestProtocol`, `ManifestBasket`, `ManifestCertificate`, `ManifestSpending`, `ManifestCounterparty` |
