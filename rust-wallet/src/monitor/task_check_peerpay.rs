...
/// PaymentToken — the decrypted message body from a BRC-29 payment
#[derive(Debug, serde::Deserialize)]
struct PaymentToken {
    #[serde(rename = "customInstructions")]
    custom_instructions: Option<PaymentInstructions>,
    transaction: Option<String>,  // Atomic BEEF as base64
    amount: Option<i64>,
}
...
pub async fn run(state: &web::Data<AppState>, _client: &reqwest::Client) -> Result<(), String> {
    ...
    for msg in &messages {
        ...
        // Parse the decrypted payment token
        let token: PaymentToken = match serde_json::from_slice(&msg.body) {
            Ok(t) => t,
            Err(e) => {
        ...
        let tx_base64 = match &token.transaction {
            Some(t) => t.clone(),
            None => {
        ...
        // Try to internalize the Atomic BEEF transaction
        // Decode base64 → parse BEEF → extract main tx → find our output → store as spendable
        let tx_bytes = match base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &tx_base64) {
            Ok(b) => b,
            Err(e) => {
                warn!("TaskCheckPeerPay: invalid base64 transaction: {}", e);
                message_ids_to_ack.push(msg.message_id.clone());
                continue;
            }
        };
