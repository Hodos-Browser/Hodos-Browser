...
const DomainPermissionsTab: React.FC = () => {
  ...
  const fetchPermissions = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch('http://localhost:3301/domain/permissions/all');
  ...
  const handleEditSave = async (settings: DomainPermissionSettings) => {
    ...
    try {
      const res = await fetch('http://localhost:3301/domain/permissions', {
        method: 'POST',
  ...
  const handleRevoke = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:3301/domain/permissions?domain=${encodeURIComponent(revokeTarget.domain)}`,
        { method: 'DELETE' }
      );
